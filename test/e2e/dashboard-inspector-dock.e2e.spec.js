...
test.describe('Inspector Dock Mode (M3)', () => {
    ...
    test('side panel docks without backdrop on desktop viewport', async ({ page, proxyServer }) => {
        ...
        await page.goto(proxyServer.url + '/dashboard#requests/live', { waitUntil: 'domcontentloaded' });

        // Inject a test request via the store
        await page.evaluate(() => {
            if (window.__DASHBOARD_STORE__ && window.__DASHBOARD_ACTIONS__) {
                window.__DASHBOARD_STORE__.dispatch(window.__DASHBOARD_ACTIONS__.addRequest({
                    id: 'dock-test-1', status: 'success', model: 'claude-sonnet-4-20250514',
                    keyIndex: 0, timestamp: Date.now()
                }));
            }
    ...
    test('side panel clears header on desktop (top equals --header-h)', async ({ page, proxyServer }) => {
        ...
        await page.evaluate(() => {
            if (window.__DASHBOARD_STORE__ && window.__DASHBOARD_ACTIONS__) {
                window.__DASHBOARD_STORE__.dispatch(window.__DASHBOARD_ACTIONS__.addRequest({
                    id: 'dock-top-1', status: 'success', model: 'test', keyIndex: 0, timestamp: Date.now()
                }));
            }
    ...
    test('--dock-right updates when inspector opens on desktop', async ({ page, proxyServer }) => {
        ...
        await page.evaluate(() => {
            if (window.__DASHBOARD_STORE__ && window.__DASHBOARD_ACTIONS__) {
                window.__DASHBOARD_STORE__.dispatch(window.__DASHBOARD_ACTIONS__.addRequest({
                    id: 'dock-test-2', status: 'success', model: 'test', keyIndex: 0, timestamp: Date.now()
                }));
            }
    ...
    test('clicking different row updates inspector content without closing', async ({ page, proxyServer }) => {
        ...
        // Inject two requests
        await page.evaluate(() => {
            var store = window.__DASHBOARD_STORE__;
            var actions = window.__DASHBOARD_ACTIONS__;
            if (store && actions) {
                ['row-a', 'row-b'].forEach(function(id) {
                    store.dispatch(actions.addRequest({
                        id: id, status: 'success', model: 'claude-sonnet-4-20250514',
                        keyIndex: 0, timestamp: Date.now()
                    }));
                });
            }
    ...
    test('bottom drawer respects --dock-right on desktop', async ({ page, proxyServer }) => {
        ...
        await page.evaluate(() => {
            if (window.__DASHBOARD_STORE__ && window.__DASHBOARD_ACTIONS__) {
                window.__DASHBOARD_STORE__.dispatch(window.__DASHBOARD_ACTIONS__.addRequest({
                    id: 'drawer-test', status: 'success', model: 'test', keyIndex: 0, timestamp: Date.now()
                }));
            }
    ...
    test('side panel uses overlay with backdrop on mobile viewport', async ({ page, proxyServer }) => {
        ...
        await page.evaluate(() => {
            if (window.__DASHBOARD_STORE__ && window.__DASHBOARD_ACTIONS__) {
                window.__DASHBOARD_STORE__.dispatch(window.__DASHBOARD_ACTIONS__.addRequest({
                    id: 'mobile-test', status: 'success', model: 'test', keyIndex: 0, timestamp: Date.now()
                }));
            }
    ...
    test('side panel top is 0 on mobile (full overlay)', async ({ page, proxyServer }) => {
        ...
        await page.evaluate(() => {
            if (window.__DASHBOARD_STORE__ && window.__DASHBOARD_ACTIONS__) {
                window.__DASHBOARD_STORE__.dispatch(window.__DASHBOARD_ACTIONS__.addRequest({
                    id: 'mobile-top-test', status: 'success', model: 'test', keyIndex: 0, timestamp: Date.now()
                }));
            }

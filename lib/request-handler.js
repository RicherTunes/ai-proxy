...
class RequestHandler extends EventEmitter {
    ...
    _createAgent() {
        ...
        this.agent.on('error', (err) => {
            this.logger?.error('HTTPS agent error', { error: err.message });
        });
    }
    ...
    async _makeProxyRequest(req, res, body, keyInfo, requestId, reqLogger, startTime, attempt, clientDisconnected = false, onProxyReq = null, useFreshConnection = false, traceAttempt = null, attemptedModels = null, traceId = null) {
        const targetPath = this.targetBasePath + req.url;

        // Apply model mapping transformation (Claude -> GLM)
        const { body: transformedBody, originalModel, mappedModel, routingDecision } = await this._transformRequestBody(
            body,
            reqLogger,
            keyInfo.index,
            req,
        ...
        if (routingDecision && routingDecision.contextOverflow) {
            ...
            if (cause === 'transient_unavailable' && transientRetryEnabled) {
                ...
                // statsAggregator?.recordContextOverflowTransient is a future extension point;
                // currently a safe no-op via optional chaining (no method on StatsAggregator).
        ...
        return new Promise((resolve) => {
            ...
            const [targetHostname, targetPortStr] = this.targetHost.split(':');
            ...
            const upstreamSpan = traceAttempt?.addSpan(SpanType.UPSTREAM_START, {
                metadata: { targetHost: this.targetHost, targetPath }
            });

...
class RequestHandler extends EventEmitter {
    ...
    async _proxyWithRetries(req, res, body, requestId, reqLogger, startTime, trace) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            ...
            // Get a key (excluding failed ones for non-socket-hangup errors)
            let keyInfo = this.keyManager.acquireKey(excludeKeys);
            ...
            if (!keyInfo) {
                ...
                if (attempt === 0 && this.requestQueue.hasCapacity()) {
                    ...
                    // Try to acquire key again after waiting
                    keyInfo = this.keyManager.acquireKey(excludeKeys);
                }

                // Still no key after queue wait
                if (!keyInfo) {
                    reqLogger?.error('No keys available', { excludedCount: excludeKeys.length });
                    if (!res.headersSent) {
                        res.writeHead(503, {
                            'content-type': 'application/json',
                            'retry-after': '2'
     ...
     */
    async _makeProxyRequest(req, res, body, keyInfo, requestId, reqLogger, startTime, attempt, clientDisconnected = false, onProxyReq = null, useFreshConnection = false, traceAttempt = null, attemptedModels = null, traceId = null) {
        // Apply model mapping transformation (Claude -> GLM)
        const { body: transformedBody, originalModel, mappedModel, routingDecision, provider: resolvedProviderName } = await this._transformRequestBody(
            body,
            reqLogger,
            keyInfo.index,
            req,
        ...
        // Resolve provider config for this request (per-provider target URL, auth, etc.)
        const providerRegistry = this.config?._providerRegistry || null;
        const providerConfig = providerRegistry
            ? providerRegistry.getProvider(resolvedProviderName || providerRegistry.getDefaultProviderName())
            : null;

        // Populate trace with provider info (once per attempt)
        if (traceAttempt && resolvedProviderName) {
            traceAttempt.metadata = traceAttempt.metadata || {};
            traceAttempt.metadata.provider = resolvedProviderName;
            traceAttempt.metadata.costTier = providerConfig?.costTier || 'metered';
        }

        // Per-request target: use provider config if available, else fall back to instance defaults
        ...
        const emitRequestToStream = (eventData) => {
            this.addRequestToStream({
                ...
                ...(resolvedProviderName ? { provider: resolvedProviderName } : {}),
                ...(providerConfig?.costTier ? { costTier: providerConfig.costTier } : {})
            });
        };
        ...
        return new Promise((resolve) => {
            ...
            // Parse port from targetHost (url.host may include :port)
            ...
            // Build auth headers per provider (GUARD-05: key isolation)
            const authHeaders = {};
            if (providerRegistry && resolvedProviderName) {
                ...
                // Backward compat: send both formats when no provider registry
                authHeaders['x-api-key'] = keyInfo.key;
                authHeaders['authorization'] = `Bearer ${keyInfo.key}`;
            }

            // Apply provider-specific extra headers (e.g., anthropic-version)

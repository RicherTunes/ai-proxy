...
// Content preview limits (used by _extractRequestContentPreview)
const MAX_PREVIEW_MESSAGES = 12;
const MAX_CHARS_PER_MESSAGE = 1600;
const MAX_PREVIEW_TOTAL_CHARS = 12000;
...
class RequestHandler extends EventEmitter {
    ...
    _extractRequestContentPreview(body) {
        ...
        const maxCharsPerMessage = MAX_CHARS_PER_MESSAGE;
        const maxTotalChars = MAX_PREVIEW_TOTAL_CHARS;
        let totalChars = 0;
        let truncated = false;
    ...
    _createAgent() {
        ...
        this.agent = new AgentClass({
            keepAlive: true,
            keepAliveMsecs: 5000,        // Frequent keepalive probes
            maxSockets: 30,              // Limit concurrent connections
            maxFreeSockets: 5,           // Fewer idle sockets (reduces stale pool)
            timeout: this.keepAliveTimeout,
            freeSocketTimeout: this.freeSocketTimeout || 15000,    // Aggressively close idle sockets (z.ai drops them sooner)
            scheduling: 'lifo'           // Reuse most recent socket (reduces stale reuse)
        });

        // Health-aware DNS: cache resolved IPs, avoid IPs that fail TCP connect
        this._healthyIPs = new Map();  // hostname -> { ips: [], lastCheck: number }
        this._badIPs = new Set();      // IPs that failed recently
    ...
    /**
     * Health-aware DNS lookup: resolve hostname and filter out IPs that recently failed.
     * Falls back to all IPs if all are marked bad (auto-recovery).
     */
    async _resolveHealthyIP(hostname) {
        const dns = require('dns').promises;
        const now = Date.now();
        const cached = this._healthyIPs?.get(hostname);

        let ips;
        if (cached && now - cached.lastCheck < 30000) {
            ips = cached.ips;
        } else {
            // Dedup concurrent DNS lookups for same hostname
            if (!this._dnsInflight) this._dnsInflight = new Map();
            let lookupPromise = this._dnsInflight.get(hostname);
            if (!lookupPromise) {
                lookupPromise = dns.resolve4(hostname).catch(() => []).finally(() => {
                    this._dnsInflight?.delete(hostname);
                });
                this._dnsInflight.set(hostname, lookupPromise);
            }
            ips = await lookupPromise;
            if (ips.length > 0) {
                if (!this._healthyIPs) this._healthyIPs = new Map();
                // Prune stale entries if cache grows too large (cap at 1000 entries)
                if (this._healthyIPs.size > 1000) {
                    const staleThreshold = now - 60000;
                    for (const [k, v] of this._healthyIPs) {
                        if (v.lastCheck < staleThreshold) this._healthyIPs.delete(k);
                    }
                }
                this._healthyIPs.set(hostname, { ips, lastCheck: now });
            }
        }
        ...
        const badIPs = this._badIPs || new Set();
        const healthy = ips.filter(ip => !badIPs.has(ip));

        if (healthy.length === 0) {
            badIPs.clear();
            this.logger?.info('All IPs marked bad, resetting', { hostname, ips });
    ...
    /**
     * Mark an IP as unhealthy (called on socket_hangup/timeout)
     */
    _markIPBad(ip) {
        if (!ip || !this._badIPs) return;
        this._badIPs.add(ip);

        // Deduplicate: cancel existing expiry timer for this IP before creating new one
        if (!this._badIPTimers) this._badIPTimers = new Map();
        const existingTimer = this._badIPTimers.get(ip);
        if (existingTimer) clearTimeout(existingTimer);

        const timer = setTimeout(() => {
            this._badIPs?.delete(ip);
            this._badIPTimers?.delete(ip);
            this.logger?.info('IP health expired, re-allowing', { ip });
        }, 60000);
        timer.unref();
        this._badIPTimers.set(ip, timer);

        this.logger?.warn('Marked IP as unhealthy', { ip, totalBad: this._badIPs.size });
    }
    ...
    async _proxyWithRetries(req, res, body, requestId, reqLogger, startTime, trace) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            ...
            // Emit request-start (first attempt) or request-retry (subsequent attempts) for live Runway visualization
            if (attempt === 0) {
                this.emit('request-start', {
                    requestId,
                    originalModel: transformResult.originalModel || null,
                    mappedModel: transformResult.mappedModel || null,
                    tier: transformResult.routingDecision?.tier || null,
                    timestamp: Date.now()
                });
            } else {
                this.emit('request-retry', {
                    ...
                    attempt,
                    mappedModel: transformResult.mappedModel || null,
                    previousModel: prevMappedModel || null,
                    tier: transformResult.routingDecision?.tier || null,
                    errorType: lastErrorType || null,
                    timestamp: Date.now()
                });
            }
            ...
            if (!keyInfo) {
                // Distinguish: no keys CONFIGURED (permanent) vs all keys BUSY (transient).
                // provider_no_keys_configured → 503 non-retryable, skip queue.
                // provider_all_keys_busy → 503 retryable, try queue.
                const hasProviderKeys = !providerFilter
                    || this.keyManager.hasKeysForProvider(providerFilter)
                    || this.keyManager.keys.some(k => k.provider === null); // untagged keys exist
                if (providerFilter && !hasProviderKeys) {
                    reqLogger?.error('No keys configured for provider', {
                        provider: providerFilter,
                        configuredProviders: Array.from(this.keyManager._providerKeyIndices.keys())
                    });
                    if (!res.headersSent) {
                        res.writeHead(503, { 'content-type': 'application/json' });
                        res.end(JSON.stringify({
                            error: `No keys configured for provider '${providerFilter}'`,
                            errorType: 'provider_no_keys_configured',
                            retryable: false,
                            requestId,
                            provider: providerFilter
                        }));
                ...
                if (!keyInfo) {
                    reqLogger?.error('All keys busy or circuits open', {
                    ...
                    if (!res.headersSent) {
                        ...
                        res.end(JSON.stringify({
                            ...
                            errorType: 'provider_all_keys_busy',
                            retryable: true,
                            retryAfter: 2,
                            requestId,
                            provider: providerFilter
    ...
    async _makeProxyRequest(req, res, body, keyInfo, requestId, reqLogger, startTime, attempt, clientDisconnected = false, onProxyReq = null, useFreshConnection = false, traceAttempt = null, attemptedModels = null, traceId = null, precomputedTransform = null) {
        ...
        if (mappedModel && this.keyManager.isModelAtCapacity?.(mappedModel, keyInfo.index)) {
            reqLogger?.warn(`Model ${mappedModel} at capacity for key #${keyInfo.index}, queuing for retry`, {
                inFlight: this.keyManager.getModelInFlight?.(mappedModel, keyInfo.index),
                attempt
            });
        ...
        const modelSlotAcquired = mappedModel ? (this.keyManager.acquireModelSlot?.(mappedModel, keyInfo.index) ?? true) : true;
        ...
        // Health-aware DNS: resolve to a known-good IP before entering the Promise
        const reqTargetHost2 = providerConfig ? providerConfig.targetHost : this.targetHost;
        const [dnsHostname] = (reqTargetHost2 || '').split(':');
        let preResolvedIP = null;
        try {
            preResolvedIP = await this._resolveHealthyIP(dnsHostname);
        } catch (_dnsErr) { /* fallback to hostname */ }
        ...
        return new Promise((resolve) => {
            ...
            const complete = (result) => {
                if (!completed) {
                    ...
                    if (mappedModel && modelSlotAcquired) {
                        this.keyManager.releaseModelSlot?.(mappedModel, keyInfo.index);
                    }
            ...
            // Provider extra headers with denylist to prevent override of structural/hop-by-hop headers
            const RESERVED_HEADERS = new Set([
                'host', 'connection', 'content-length', 'transfer-encoding',
                'x-api-key', 'authorization', 'x-request-id',
                // Hop-by-hop headers (RFC 2616 Section 13.5.1) — prevent re-injection via extraHeaders
                'keep-alive', 'proxy-authenticate', 'proxy-authorization',
                'proxy-connection', 'te', 'trailer', 'upgrade',
                // Security: prevent admin token and cookie leakage
            ...
            // Use pre-resolved healthy IP (resolved before Promise to avoid async-in-sync)
            let resolvedHostname = targetHostname;
            let resolvedIP = preResolvedIP;
            if (resolvedIP) resolvedHostname = resolvedIP;
            ...
            const options = {
                ...
                headers: {
                    ...headers,
                    'host': reqTargetHost,   // Keep original Host header for TLS SNI
                    ...authHeaders,
                    ...extraHeaders,
                    'connection': 'keep-alive',
                    'x-request-id': requestId
                },
                // When connecting to raw IP, need servername for TLS SNI
                ...(resolvedIP ? { servername: targetHostname } : {})
            };
            ...
            const proxyReq = requestFn(options, (proxyRes) => {
                ...
                // Rate limit sync: feed header observations for dynamic baseline updates
                if (mappedModel && proxyRes.headers) {
                    this.rateLimitSync?.recordHeaders?.(mappedModel, proxyRes.headers);
                }
            ...
            proxyReq.on('timeout', () => {
                ...
                // Mark the IP as bad on timeout (likely dead upstream)
                const timedOutIP = proxyReq.socket?.remoteAddress || resolvedIP;
                if (timedOutIP) this._markIPBad(timedOutIP);
            ...
            proxyReq.on('error', (err) => {
                ...
                if (errorType === 'socket_hangup') {
                    // Mark the IP as bad so future requests avoid it
                    const failedIP = proxyReq.socket?.remoteAddress || resolvedIP;
                    if (failedIP) this._markIPBad(failedIP);

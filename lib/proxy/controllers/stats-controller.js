 ...
 * Prometheus label values are enclosed in double quotes and must escape
 * backslashes, double quotes, and newlines.
 * @param {string} value - Raw label value
 * @returns {string} Escaped label value (without surrounding quotes)
 */
function escapePromLabel(value) {
    ...
    return value
        ...
        .replace(/\n/g, '\\n')
        .replace(/\r/g, '\\r')
        .replace(/\}/g, '');   // Strip } to prevent label injection
}
...
class StatsController {
    ...
    handleMetrics(req, res) {
        ...
        // Prefer CostTracker's tracked cost (per-model accurate) over re-derivation
        const trackedCost = this._costTracker?.getStats?.('allTime');
        if (trackedCost && trackedCost.cost > 0) {
            lines.push(`glm_proxy_cost_total ${trackedCost.cost.toFixed(6)}`);
        } else {
            // Fallback: derive from aggregate tokens × default rates
            const costRates = this._costTracker?.rates || {};
            const inputRate = costRates.inputTokenPer1M || 3.00;
            const outputRate = costRates.outputTokenPer1M || 15.00;
            const inputCost = ((tokens.totalInputTokens || 0) / 1_000_000) * inputRate;
            const outputCost = ((tokens.totalOutputTokens || 0) / 1_000_000) * outputRate;
        ...
        if (this._tenantManager) {
            ...
            if (tenantStats.enabled && tenantStats.tenants) {
                ...
                Object.values(tenantStats.tenants).forEach(tenant => {
                    const safeTenant = escapePromLabel(tenant.tenantId);
                    const strictMode = tenant.strictMode || false;
                    lines.push(`glm_proxy_tenant_info{tenant="${safeTenant}",strict_mode="${strictMode}"} 1`);
                });
                lines.push('');
                ...
                Object.values(tenantStats.tenants).forEach(tenant => {
                    const safeTenant = escapePromLabel(tenant.tenantId);
                    lines.push(`glm_proxy_tenant_requests_total{tenant="${safeTenant}"} ${tenant.requestCount || 0}`);
                });
                lines.push('');
                ...
                Object.values(tenantStats.tenants).forEach(tenant => {
                    const safeTenant = escapePromLabel(tenant.tenantId);
                    lines.push(`glm_proxy_tenant_keys_total{tenant="${safeTenant}"} ${tenant.keyCount || 0}`);
                });
                lines.push('');
                ...
                Object.values(tenantStats.tenants).forEach(tenant => {
                    const safeTenant = escapePromLabel(tenant.tenantId);
                    lines.push(`glm_proxy_tenant_errors_total{tenant="${safeTenant}"} ${tenant.errorCount || 0}`);
                });
                lines.push('');
            }
        }
    ...
    handleReload(req, res) {
        ...
        if (result) {
            ...
            sendJson(res, 500, { success: false, error: 'Failed to reload keys' });
        }
    }

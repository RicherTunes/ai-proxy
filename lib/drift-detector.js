...
class DriftDetector {
    ...
    _recordDrift(drift) {
        ...
        // Store in memory for testing (bounded to prevent leaks)
        this._driftEvents.push({
            ...drift,
            timestamp: Date.now()
        });

        // Hard cap: trim to most recent 500 when exceeding 1000
        if (this._driftEvents.length > 1000) {
            this._driftEvents = this._driftEvents.slice(-500);
        }
    }

...
const DEFAULT_CONFIG = {
    ...
    // Target API (default provider — overridden by providers config if present)
    targetHost: 'api.z.ai',
    ...
    // Rate Limit Sync — dynamically updates per-model concurrency baselines
    // from upstream z.ai response headers (capped at MAX_SANE_CONCURRENCY).
    // Runs on primary process only, persists to disk for restart survival.
    // NOTE: x-ratelimit-limit carries RPM, not concurrency. Header observation
    // is filtered by per-model ceiling (static+maxAboveStatic) and global cap.
    // Ceiling probe is DISABLED because there is no reliable upstream concurrency signal.
    rateLimitSync: {
        enabled: true,
        tickIntervalMs: 30000,           // Evaluate observations every 30s
        quorumSize: 3,                   // Require N consistent header values before updating
        ceilingProbeEnabled: false,      // DISABLED: z.ai headers are RPM, not concurrency
        ceilingProbeCleanTicks: 10,      // AIMD clean ticks before probing
        ceilingProbeStep: 1,             // Raise by +1 per probe
        ceilingProbeMaxAboveStatic: 5,   // Safety cap above original KNOWN_GLM value
        persistFile: 'rate-limit-cache.json',
        staleThresholdMs: 86400000       // 24h — cached limits older than this fall back to static
    },
    ...
    // Upstream Health Monitor — automatic failover endpoints
    upstreamFallbacks: [
    ...
    adminAuth: {
        ...
        sessionCookieName: 'glm_admin_session',
    ...
    security: {
        ...
        modelDiscovery: {
            ...
            probeDelayMs: 200,        // Delay between model probes (ms)
            probeTimeoutMs: 10000,    // Per-probe timeout (ms)
            persistFile: 'model-discovery-cache.json',  // Cache file for discovered models
            probeOnStartup: false     // Probe all known + candidate models on server start
        }
    }
};
...
class Config {
    ...
    _validate() {
        ...
        // Enum-like field validation (warnings, not errors — config still works with defaults)
        const VALID_LOG_LEVELS = ['DEBUG', 'INFO', 'WARN', 'ERROR'];
        if (this.config.logLevel && !VALID_LOG_LEVELS.includes(this.config.logLevel)) {
            warnings.push(`Invalid logLevel: '${this.config.logLevel}' (valid: ${VALID_LOG_LEVELS.join(', ')})`);
        }

        const VALID_LOG_FORMATS = ['text', 'json'];
        if (this.config.logFormat && !VALID_LOG_FORMATS.includes(this.config.logFormat)) {
            warnings.push(`Invalid logFormat: '${this.config.logFormat}' (valid: ${VALID_LOG_FORMATS.join(', ')})`);
        }

        const VALID_ADAPTIVE_MODES = ['observe_only', 'enforce'];
        if (this.config.adaptiveConcurrency?.mode &&
            !VALID_ADAPTIVE_MODES.includes(this.config.adaptiveConcurrency.mode)) {
            warnings.push(`Invalid adaptiveConcurrency.mode: '${this.config.adaptiveConcurrency.mode}' (valid: ${VALID_ADAPTIVE_MODES.join(', ')})`);
        }
    ...
    get upstreamFallbacks() { return this.config.upstreamFallbacks; }
}

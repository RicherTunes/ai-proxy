...
const DEFAULT_CONFIG = {
    ...
    // Multi-provider configuration (Month 5-6 roadmap)
    // When empty/null, a default z.ai provider is created from targetHost/BasePath/Protocol above.
    // Each provider defines its own target, auth scheme, and transformation rules.
    providers: null,
...
class Config {
    ...
    _initializeProviderRegistry() {
        ...
        if (providersConfig) {
            ...
            // Build default provider from global target config (backward compat)
        ...
        // Also attach to raw config for consumers that receive config object directly

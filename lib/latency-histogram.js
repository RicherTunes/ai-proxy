...
class LatencyHistogram {
    ...
    record(latencyMs) {
        ...
        // If buffer is full, the oldest item at head will be evicted — decrement its bucket
        if (this.dataPoints.size === this.dataPoints.capacity) {
            const evicted = this.dataPoints.buffer[this.dataPoints.head];
            if (evicted) {
                const oldBucket = this._findBucket(evicted.latencyMs);
                const oldLabel = this._getBucketLabel(oldBucket);
                const oldCount = this._bucketCounts.get(oldLabel) || 0;
                if (oldCount > 0) {
                    this._bucketCounts.set(oldLabel, oldCount - 1);
                }
            }
        }

        // Push new item (RingBuffer handles position and size)
        this.dataPoints.push({ latencyMs, timestamp });

        // Increment bucket for the new item
        const newBucket = this._findBucket(latencyMs);
        const newLabel = this._getBucketLabel(newBucket);
        this._bucketCounts.set(newLabel, (this._bucketCounts.get(newLabel) || 0) + 1);
    }
    ...
    getHistogram(timeRange = '15m') {
        ...
        const allPoints = this.dataPoints.toArray();
        const filteredPoints = cutoffMs === 0 ? allPoints : allPoints.filter(p => p.timestamp >= cutoffMs);

        // Build histogram bucket counts
        const histogram = {};
        const usePrecomputed = cutoffMs === 0;

        if (usePrecomputed) {
            // Fast path: use incrementally maintained _bucketCounts

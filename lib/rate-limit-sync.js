 ...
 * response headers (x-ratelimit-limit, capped at MAX_SANE_CONCURRENCY) and optionally
...
const { MAX_SANE_CONCURRENCY } = require('./model-discovery');
...
class RateLimitSync {
    ...
    recordHeaders(model, headers) {
        ...
        // The x-ratelimit-limit header often carries RPM (requests-per-minute),
        // not concurrency.  Values above MAX_SANE_CONCURRENCY are almost certainly
        // RPM and must not pollute the concurrency baseline.
        if (limit > MAX_SANE_CONCURRENCY) {
            this._logger.debug('RateLimitSync: header value likely RPM, ignoring', {
                model, headerValue: limit, maxSane: MAX_SANE_CONCURRENCY
            });
            return;
        }
    ...
    _tick() {
        ...
        for (const [model, w] of windows) {
            ...
            // Capture originalStatic on first encounter to prevent sliding baseline
            // (without this, getStaticModelLimit returns the already-bumped value,
            // causing the cap to slide upward on every tick).
            if (!this._originalStaticLimits.has(model)) {
                const base = this._keyManager?.getStaticModelLimit?.(model) || w.staticMax;
                this._originalStaticLimits.set(model, base);
            }
            const originalStatic = this._originalStaticLimits.get(model);
    ...
    _applyDiscoveredLimit(model, newLimit, source) {
        ...
        // Absolute sanity guard — never propagate values above the global cap
        if (newLimit > MAX_SANE_CONCURRENCY) {
            this._logger.warn('RateLimitSync: discovered limit exceeds global sanity cap, ignoring', {
                model, newLimit, maxSane: MAX_SANE_CONCURRENCY, source
            });
            return;
        }

        // Per-model ceiling: don't exceed originalStatic + maxAboveStatic.
        // The x-ratelimit-limit header carries RPM (requests-per-minute), not
        // concurrency.  Low RPM values (e.g. 10, 15) can slip under the global
        // cap but still far exceed the real concurrency limit.  This per-model
        // guard uses the stress-tested static baseline as the anchor.
        const originalStatic = this._originalStaticLimits.get(model);
        if (originalStatic !== undefined) {
            const perModelMax = originalStatic + this._config.ceilingProbeMaxAboveStatic;
            if (newLimit > perModelMax) {
                this._logger.debug('RateLimitSync: discovered limit exceeds per-model ceiling, ignoring', {
                    model, newLimit, originalStatic, perModelMax, source
                });
                return;
            }
        }
    ...
    _applyCachedBaselines() {
        ...
        for (const [model, baseline] of this._baselines) {
            ...
            // Defense in depth: reject cached baselines above sanity cap
            // (poisoned caches from before the RPM-vs-concurrency fix)
            if (baseline.concurrency > MAX_SANE_CONCURRENCY) {
                this._logger.warn('RateLimitSync: cached baseline exceeds sanity cap, discarding', {
                    model,
                    cachedConcurrency: baseline.concurrency,
                    maxSane: MAX_SANE_CONCURRENCY
                });
                this._baselines.delete(model);
                continue;
            }
            ...
            // Capture original static before any modification
            if (!this._originalStaticLimits.has(model) && currentStatic !== undefined) {
                this._originalStaticLimits.set(model, currentStatic);
            }

            // Route through _applyDiscoveredLimit so all guards apply uniformly
            // (global cap, per-model ceiling, propagation to all subsystems)
            this._applyDiscoveredLimit(model, baseline.concurrency, 'cached');
        }
    }
            ...
            if (data.baselines && typeof data.baselines === 'object') {
                for (const [model, baseline] of Object.entries(data.baselines)) {
                    if (baseline && typeof baseline.concurrency === 'number' && baseline.concurrency >= 1
                        && baseline.concurrency <= MAX_SANE_CONCURRENCY) {
                        this._baselines.set(model, {

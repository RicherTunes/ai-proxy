...
/**
 * Sanitize a tenant ID: strip control characters, quotes, braces, and enforce length limit.
 * @param {string} tenantId - Raw tenant ID from header
 * @returns {string} Sanitized tenant ID, falls back to DEFAULT_TENANT_ID if empty
 */
function sanitizeTenantId(tenantId) {
    if (typeof tenantId !== 'string') return DEFAULT_TENANT_ID;
    // Strip control characters, path separators, quotes, braces, and backslashes
    let sanitized = tenantId.replace(/[\x00-\x1f\x7f"{}\\\/]/g, '');
    // Prevent path traversal via consecutive dots
    sanitized = sanitized.replace(/\.{2,}/g, '.');
    // Enforce length limit
    if (sanitized.length > MAX_TENANT_ID_LENGTH) {
        sanitized = sanitized.slice(0, MAX_TENANT_ID_LENGTH);
    }
...
class TenantManager {
    ...
    getTenantFromRequest(req) {
        ...
        const rawTenantId = req.headers[this.tenantHeader];
        const tenantId = rawTenantId ? sanitizeTenantId(rawTenantId) : this.defaultTenantId;
        const context = this.tenants.get(tenantId);

        // Track stats
        this.globalStats.totalRequests++;

        // Only track new tenant IDs if under the cap (prevent unbounded growth)
        if (tenantId in this.globalStats.requestsByTenant) {
            this.globalStats.requestsByTenant[tenantId]++;
        } else if (Object.keys(this.globalStats.requestsByTenant).length < 10000) {
            this.globalStats.requestsByTenant[tenantId] = 1;
        } else {

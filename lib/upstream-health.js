...
// Recovery thresholds: how many consecutive successes before switching state
const PRIMARY_RECOVERY_THRESHOLD = 2;
const FALLBACK_RECOVERY_THRESHOLD = 5;
// Minimum time (ms) after failover before switching back to primary
const MIN_FAILOVER_AGE_MS = 60000;
...
class UpstreamHealthMonitor extends EventEmitter {
     ...
     */
    async _probe() {
        // Guard against concurrent probes (failover validation is async)
    ...
    async _recordProbeResult(probeState, reason, affectedIPs) {
        ...
        if (probeState === 'healthy') {
            ...
            if (prevState !== 'healthy') {
                ...
                const recoveryThreshold = this.activeEndpoint.isPrimary ? PRIMARY_RECOVERY_THRESHOLD : FALLBACK_RECOVERY_THRESHOLD;
                if (this.consecutiveSuccesses >= recoveryThreshold || prevState === 'degraded') {
                    // Don't switch back to primary if we failed over less than MIN_FAILOVER_AGE_MS ago
                    const failoverAge = this._lastFailoverAt ? Date.now() - this._lastFailoverAt : Infinity;
                    if (!this.activeEndpoint.isPrimary && failoverAge < MIN_FAILOVER_AGE_MS) {

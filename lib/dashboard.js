...
const ASSET_VERSION = _pkgVersion + '-' + crypto.createHash('sha256').update(_pkgVersion).digest('hex').slice(0, 8);
...
function svgIcon(name, size = 16, cls = '') {
    ...
    return `<svg class="icon${classAttr}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${pathData}</svg>`;
}
...
<body>
    ...
    <div class="modal-overlay" id="shortcutsModal" role="dialog" aria-modal="true" aria-labelledby="shortcutsModalTitle" data-action="close-modal" data-modal="shortcuts">
        <div class="modal">
    ...
    <!-- Key Override Modal -->
    <div class="modal-overlay" id="keyOverrideModal" role="dialog" aria-modal="true" aria-labelledby="keyOverrideTitle" data-action="close-modal" data-modal="key-override">
        <div class="modal key-override-modal">
            <div class="modal-header">
                <h2 id="keyOverrideTitle">Configure Routing Overrides</h2>
                <button class="modal-close" aria-label="Close" data-action="close-key-override-modal">&times;</button>
            </div>
            <div class="modal-body">
                ...
                <div class="override-section">
                    ...
                    <div class="add-override-form" id="addOverrideForm">
                        <input type="text" id="newClaudeModel" placeholder="Claude model (e.g., claude-sonnet-4-5-20250929)" aria-label="Claude model name">
                        <input type="text" id="newGlmModel" placeholder="GLM model (e.g., glm-4.7)" aria-label="GLM model name">
    ...
    <div class="modal-overlay" id="restartModal" role="dialog" aria-modal="true" aria-labelledby="restartModalTitle" data-action="close-modal" data-modal="restart">
        <div class="modal">
            <div class="modal-header">
                <h2 id="restartModalTitle">Restart Proxy</h2>
                <button class="modal-close" aria-label="Close" data-action="close-restart-modal">&times;</button>
            </div>
            <div class="modal-body">
                <p id="restartModalDesc" class="modal-description">
                    This will gracefully restart the proxy. In-flight requests will be drained first.
                </p>
                <div id="restartStatus" style="display:none">
                    <div class="state-loading">
                        <div class="spinner"></div>
                        <span id="restartStatusText">Restarting...</span>
                    </div>
                </div>
            </div>
            <div class="modal-footer" id="restartModalFooter">
                <button class="btn btn-secondary" data-action="close-restart-modal">Cancel</button>
                <button class="btn btn-danger" id="restartConfirmBtn" data-action="confirm-restart">Restart Now</button>
            </div>
        </div>
    </div>
    ...
    <header class="sticky-header" data-testid="sticky-header">
        ...
        <div class="header-section header-section--branding">
            ...
            <div class="connection-status" id="connectionStatus">
                <span class="connection-dot connected" id="connectionDot" data-testid="connection-dot" aria-hidden="true"></span>
                <span class="visually-hidden" id="connectionStatusText" aria-live="polite">Connected</span>
                <span class="connection-inflight" id="connectionInFlight" title="In-flight requests">0</span>
                <canvas class="header-sparkline" id="headerSparkline" width="60" height="16" title="Requests/sec (last 60s)"></canvas>
                <span class="upstream-badge" id="headerUpstreamBadge" style="display:none" title="Upstream health"></span>
            ...
            <span class="badge badge-warning text-xs" id="headerCostAlert" style="display:none;" title="Cost budget alert">$!</span>
        </div>
        ...
        <div class="header-section header-section--primary">
            ...
            <div class="time-range-selector" id="timeRangeSelector" data-testid="time-range">
                ...
                <div class="time-range-dropdown" id="timeRangeDropdown" role="listbox" aria-label="Select time range">
                    <button class="time-range-dropdown-item" data-range="5m" data-action="set-time-range" role="option">5 minutes</button>
                    <button class="time-range-dropdown-item" data-range="15m" data-action="set-time-range" role="option">15 minutes</button>
                    <button class="time-range-dropdown-item active" data-range="1h" data-action="set-time-range" role="option" aria-selected="true">1 hour</button>
        ...
        <div class="header-section header-section--actions">
            ...
            <button class="btn btn-danger btn-small" id="restartBtn" data-action="control-restart" data-admin="true" data-testid="restart-btn" title="Restart proxy process">Restart</button>
            ...
            <div class="header-toolbar-promoted" id="headerToolbarPromoted">
                <button class="btn-icon-toolbar theme-toggle" data-action="toggle-theme" title="Toggle theme (D)" aria-label="Toggle theme" data-testid="theme-toggle">
                ...
                <button class="btn-icon-toolbar export-btn" data-action="export-data" title="Export data (E)" aria-label="Export data">
                    ${svgIcon('download')}
                </button>
                <button class="btn-icon-toolbar share-url-btn" id="shareUrlBtn" data-action="share-url" title="Copy shareable link" aria-label="Copy shareable link">
                    ${svgIcon('external-link')}
                </button>
                <button class="btn-icon-toolbar" data-action="show-shortcuts-modal" title="Keyboard shortcuts (?)" aria-label="Keyboard shortcuts">
                    ?
                </button>
            ...
            <div class="overflow-menu-container" id="overflowMenuContainer">
                ...
                <div class="overflow-menu-dropdown" id="overflowMenuDropdown" role="menu" aria-label="Additional options">
                    ...
                    <div class="overflow-menu-divider"></div>
                    <button class="overflow-menu-item overflow-menu-item--danger" data-action="control-restart" data-admin="true" role="menuitem">
                        Restart Proxy
                    </button>
    ...
    <nav class="page-nav" data-testid="page-nav" role="tablist" aria-label="Dashboard pages">
        <button class="page-nav-btn active" data-page="overview" data-action="switch-page" role="tab" aria-selected="true" aria-current="page" aria-controls="page-overview" tabindex="0">Overview</button>
        <button class="page-nav-btn" data-page="routing" data-action="switch-page" role="tab" aria-selected="false" aria-controls="page-routing" tabindex="-1">Routing</button>
        <button class="page-nav-btn" data-page="requests" data-action="switch-page" role="tab" aria-selected="false" aria-controls="page-requests" tabindex="-1">Requests</button>
        <button class="page-nav-btn" data-page="system" data-action="switch-page" role="tab" aria-selected="false" aria-controls="page-system" tabindex="-1">Diagnostics</button>
    </nav>
    ...
    <!-- Welcome Banner (hidden once data arrives) -->
    <div class="overview-welcome" id="overviewWelcomeBanner" data-belongs-to="overview">
        <div class="welcome-content">
            <h3>GLM Proxy Dashboard</h3>
            <p>Send requests through the proxy to see live metrics, routing decisions, and performance data.</p>
            <div class="welcome-hints">
                <span class="welcome-hint">Proxy is running at <code>127.0.0.1:18765</code></span>
                <span class="welcome-hint">Press <kbd>?</kbd> for keyboard shortcuts</span>
                <span class="welcome-hint">Right-click requests for quick actions</span>
            </div>
        </div>
    </div>
    ...
    <div class="dashboard-grid page-section" data-belongs-to="overview">
        ...
        <div class="column-section">
            ...
            <div class="chart-card">
                ...
                <button class="fullscreen-btn" data-action="toggle-fullscreen" aria-label="Toggle fullscreen" data-chart="requestChartContainer">${svgIcon('maximize-2')}</button>
            ...
            <div class="chart-card">
                ...
                <button class="fullscreen-btn" data-action="toggle-fullscreen" aria-label="Toggle fullscreen" data-chart="latencyChartContainer">${svgIcon('maximize-2')}</button>
            ...
            <div class="chart-card">
                ...
                <button class="fullscreen-btn" data-action="toggle-fullscreen" aria-label="Toggle fullscreen" data-chart="errorChartContainer">${svgIcon('maximize-2')}</button>
        ...
        <div class="column-section">
            ...
            <section class="cost-panel" id="costPanel">
                ...
                <div class="cost-stats">
                    ...
                    <div class="cost-stat">
                        <div class="label">Cost per 1K Tokens</div>
                        <div class="value" id="costPer1kTokens">$0.00</div>
                    </div>
                    <div class="cost-stat">
                ...
                <div class="chart-card" id="costChartCard" style="margin-top: 8px; padding: 8px; display: none;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <h4 class="text-sm text-secondary mb-4">Cost Over Time (by Tier)</h4>
                        <div class="chart-nav">
                    ...
                    <div class="chart-container" data-chart="costTime" style="height: clamp(140px, 14vw, 200px);">
                        <canvas id="costTimeChart"></canvas>
                        <div class="chart-loading"><div class="skeleton-spinner"></div><div class="skeleton-text">Loading cost data...</div></div>
                    </div>
                </div>
            ...
            <div class="chart-card" id="distChartCard">
                ...
                <button class="fullscreen-btn" data-action="toggle-fullscreen" aria-label="Toggle fullscreen" data-chart="distChartContainer">${svgIcon('maximize-2')}</button>
    ...
    <section class="model-selection-section page-section" id="modelSelectionSection" data-testid="model-selection-section"
        ...
        <div class="model-selection-header section-header flex-wrap px-4">
            <h2 class="m-0 text-heading-md">Model Selection</h2>
    ...
    <section class="model-routing-panel" id="modelRoutingPanel" data-testid="model-routing-panel">
        <h3>
            ...
            <div class="routing-status">
                ...
                <button type="button" data-action="noop" title="See docs/model-routing.md in the repository" class="btn-link ml-auto text-sm text-accent">Learn more</button>
            </div>
        </h3>
            ...
            <div id="tierBuilderContainer" class="tier-builder" data-belongs-to="routing">
                ...
                <div class="models-bank">
                    <div class="models-bank-header">
                        ...
                        <button class="btn btn-small btn-secondary" id="modelsRefreshBtn"
                                data-action="refresh-models"
                                title="Probe z.ai for new and updated models">
                            &#x21BB; Refresh
                        </button>
                    ...
                    <div class="models-bank-status" id="modelsRefreshStatus">
                        <span class="models-refresh-text" id="modelsRefreshText"></span>
                    </div>
                    <div class="models-bank-list" id="modelsBankList">
                        <div class="tier-builder-empty">Loading models...</div>
                    </div>
                    <div class="models-bank-custom" id="modelsCustomProbe">
                        <input type="text" id="customModelInput" placeholder="e.g. glm-5-turbo"
                               class="input-small" aria-label="Custom model ID to probe">
                        <button class="btn btn-small" id="customModelProbeBtn"
                                title="Check if this model exists on z.ai">Probe</button>
                    </div>
                </div>
    ...
    <div id="liveFlowContainer" class="live-flow-container page-section" data-belongs-to="routing"
         role="region" aria-label="Per-request routing flow visualization">
        <div class="live-flow-header">
            <h4>Request Flow</h4>
            <span class="live-flow-status" id="liveFlowStatus" aria-live="polite">Connecting...</span>
        </div>
        <div class="runway-viz" id="runwayViz">
            ...
            <div class="runway-jump" id="runwayJump" style="display:none">
                <button class="runway-jump-btn" id="runwayJumpBtn" aria-label="Jump to latest requests">&#x25BC; Latest</button>
            </div>
            <div class="runway-empty" id="runwayEmpty" role="status" aria-label="No active requests">
                <svg class="runway-empty-svg" viewBox="0 0 400 100" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <!-- Client terminal icon -->
                    <g class="runway-svg-client" transform="translate(40, 25)">
                        <rect x="0" y="0" width="36" height="28" rx="3" fill="none" stroke="var(--text-secondary)" stroke-width="1.5"/>
                        <line x1="4" y1="8" x2="20" y2="8" stroke="var(--warning, #f59e0b)" stroke-width="1.5" stroke-linecap="round" opacity="0.6"/>
                        <line x1="4" y1="13" x2="14" y2="13" stroke="var(--text-secondary)" stroke-width="1" stroke-linecap="round" opacity="0.4"/>
                        <line x1="4" y1="17" x2="24" y2="17" stroke="var(--text-secondary)" stroke-width="1" stroke-linecap="round" opacity="0.3"/>
                        <rect x="8" y="30" width="20" height="3" rx="1" fill="var(--text-secondary)" opacity="0.3"/>
                        <text x="18" y="48" text-anchor="middle" fill="var(--text-secondary)" font-size="8" font-family="inherit" opacity="0.5">proxy</text>
                    </g>
                    <line x1="90" y1="39" x2="270" y2="39" stroke="var(--border)" stroke-width="1" stroke-dasharray="4,4" opacity="0.5"/>
                    <circle class="runway-dot-send" r="3" fill="var(--warning, #f59e0b)" opacity="0.7">
                        <animateMotion dur="2.5s" repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear">
                            <mpath href="#runway-path-forward"/>
                        </animateMotion>
                    </circle>
                    <circle class="runway-dot-send" r="2.5" fill="var(--warning, #f59e0b)" opacity="0.4">
                        <animateMotion dur="2.5s" repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear" begin="0.8s">
                            <mpath href="#runway-path-forward"/>
                        </animateMotion>
                    </circle>
                    <circle class="runway-dot-recv" r="3" fill="var(--success, #10b981)" opacity="0.7">
                        <animateMotion dur="2.5s" repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear" begin="1.2s">
                            <mpath href="#runway-path-reverse"/>
                        </animateMotion>
                    </circle>
                    <circle class="runway-dot-recv" r="2.5" fill="var(--success, #10b981)" opacity="0.4">
                        <animateMotion dur="2.5s" repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear" begin="2s">
                            <mpath href="#runway-path-reverse"/>
                        </animateMotion>
                    </circle>
                    <defs>
                        <path id="runway-path-forward" d="M 90 39 L 270 39" fill="none"/>
                        <path id="runway-path-reverse" d="M 270 39 L 90 39" fill="none"/>
                    </defs>
                    <g class="runway-svg-server" transform="translate(280, 18)">
                        <rect x="0" y="0" width="40" height="42" rx="3" fill="none" stroke="var(--text-secondary)" stroke-width="1.5"/>
                        <line x1="0" y1="14" x2="40" y2="14" stroke="var(--text-secondary)" stroke-width="0.75" opacity="0.3"/>
                        <line x1="0" y1="28" x2="40" y2="28" stroke="var(--text-secondary)" stroke-width="0.75" opacity="0.3"/>
                        <circle cx="8" cy="7" r="2" fill="var(--success, #10b981)" opacity="0.6"/>
                        <circle cx="8" cy="21" r="2" fill="var(--success, #10b981)" opacity="0.4"/>
                        <circle cx="8" cy="35" r="2" fill="var(--text-secondary)" opacity="0.3"/>
                        <line x1="14" y1="7" x2="34" y2="7" stroke="var(--text-secondary)" stroke-width="1" stroke-linecap="round" opacity="0.3"/>
                        <line x1="14" y1="21" x2="28" y2="21" stroke="var(--text-secondary)" stroke-width="1" stroke-linecap="round" opacity="0.3"/>
                        <line x1="14" y1="35" x2="30" y2="35" stroke="var(--text-secondary)" stroke-width="1" stroke-linecap="round" opacity="0.2"/>
                        <text x="20" y="56" text-anchor="middle" fill="var(--text-secondary)" font-size="8" font-family="inherit" opacity="0.5">z.ai</text>
                    </g>
                    <text x="180" y="32" text-anchor="middle" fill="var(--warning, #f59e0b)" font-size="7" font-family="inherit" opacity="0.5">request</text>
                    <text x="180" y="52" text-anchor="middle" fill="var(--success, #10b981)" font-size="7" font-family="inherit" opacity="0.5">response</text>
                </svg>
                <span class="state-message runway-empty-text">Waiting for requests</span>
            </div>
            <div class="runway-footer" id="runwayFooter"></div>
            <div class="runway-legend">
                <span class="runway-legend-item"><span class="runway-legend-bar send"></span>sending</span>
                <span class="runway-legend-item"><span class="runway-legend-bar recv"></span>receiving</span>
                <span class="runway-legend-sep"></span>
                <span class="runway-legend-item"><span class="runway-legend-pip heavy"></span>heavy</span>
                <span class="runway-legend-item"><span class="runway-legend-pip medium"></span>medium</span>
                <span class="runway-legend-item"><span class="runway-legend-pip light"></span>light</span>
            </div>
        </div>
    </div>
        ...
        <div class="routing-tab-content">
            ...
            <div class="routing-tab-panel" data-routing-panel="overrides"
                ...
                <div class="flex gap-8 mt-8">
                    <input type="text" id="routingOverrideKey" placeholder="Model key or *" class="override-input" aria-label="Override model key">
                    <input type="text" id="routingOverrideModel" placeholder="Target model" class="override-input" aria-label="Override target model">
                ...
                <div class="rule-builder-form rule-builder-row">
                    <div class="flex-1 min-w-120">
                        ...
                        <input type="text" id="ruleModelGlob" placeholder="e.g. claude-3-haiku-*" class="inline-input-base" aria-label="Model glob pattern">
                    </div>
                    <div class="min-w-80">
                        <label class="form-label-xs">Max Tokens &ge;</label>
                        <input type="number" id="ruleMaxTokens" placeholder="--" class="narrow-input-80" aria-label="Minimum max tokens">
                    </div>
                    <div class="min-w-80">
                        <label class="form-label-xs">Messages &ge;</label>
                        <input type="number" id="ruleMessages" placeholder="--" class="narrow-input-80" aria-label="Minimum message count">
                    </div>
                    ...
                    <div class="min-w-90">
                        ...
                        <select id="ruleTier" class="inline-input-base" aria-label="Target tier">
    ...
    <section class="info-row page-section" data-belongs-to="system">
        <div class="upstream-health-panel" id="upstreamHealthPanel">
            <h4>Upstream Health</h4>
            <div class="upstream-health-status" id="upstreamHealthStatus">
                <div class="upstream-health-indicator">
                    <span class="upstream-dot" id="upstreamDot"></span>
                    <span class="upstream-state" id="upstreamState">Checking...</span>
                    <span class="upstream-endpoint" id="upstreamEndpoint"></span>
                </div>
                <div class="upstream-health-details" id="upstreamHealthDetails"></div>
            </div>
            <div class="upstream-ips" id="upstreamIPs"></div>
            <div class="upstream-outage" id="upstreamOutage" style="display:none">
                <div class="upstream-outage-header">
                    <span class="upstream-outage-icon">&#x26A0;</span>
                    <span class="upstream-outage-title">Outage Detected</span>
                    <span class="upstream-outage-duration" id="upstreamOutageDuration"></span>
                </div>
                <div class="upstream-outage-details" id="upstreamOutageDetails"></div>
            </div>
            <div class="upstream-history" id="upstreamHistory"></div>
            <div class="upstream-config">
                <h5>Failover Endpoints</h5>
                <div class="upstream-failover-list" id="upstreamFailoverList"></div>
                <div class="upstream-config-note">
                    Failover activates automatically when the primary endpoint is unreachable.
                    The proxy will switch back when the primary recovers.
                </div>
            </div>
        </div>
    </section>
    ...
    <!-- Model Discovery -->
    <section class="info-row page-section" data-belongs-to="system">
        <div class="info-card" id="modelDiscoveryCard">
            <h3>
                <span class="icon-wrapper">${svgIcon('search', 16, 'info-card-icon')}</span>
                Model Discovery
                <button class="btn btn-primary btn-small" id="systemRefreshModelsBtn">Refresh All Models</button>
            </h3>
            <div class="info-grid">
                <div class="info-item">
                    <div class="label">Status</div>
                    <div class="value" id="discoveryStatusValue">Idle</div>
                </div>
                <div class="info-item">
                    <div class="label">Last Probed</div>
                    <div class="value" id="discoveryLastProbe">Never</div>
                </div>
                <div class="info-item">
                    <div class="label">Known Models</div>
                    <div class="value" id="discoveryKnownCount">-</div>
                </div>
                <div class="info-item">
                    <div class="label">Discovered</div>
                    <div class="value" id="discoveryFoundCount">0</div>
                </div>
            </div>
            <div class="discovery-results-table" id="discoveryResultsContainer" style="margin-top: var(--spacing-sm);">
                <table class="mini-table" id="discoveryResultsTable">
                    <thead>
                        <tr>
                            <th>Model ID</th>
                            <th>Status</th>
                    ...
                    <tbody id="discoveryResultsBody">
                        <tr><td colspan="4" class="text-muted">No probe results yet. Click "Refresh All Models" to discover available models.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <section class="info-row page-section" data-belongs-to="system">
        <div class="info-card" id="errorBreakdownCard">
            <h3>
                ...
                Error Breakdown
                <button class="btn btn-secondary btn-small text-xs" data-action="kpi-navigate" data-page="requests" data-tab="traces">View Traces</button>
        ...
        <div class="info-card">
            ...
            <div class="info-grid">
                ...
                <div class="info-item">
                    <div class="label">Context Overflow (Transient)</div>
                    <div class="value" id="contextOverflowTransient">0</div>
                </div>
            </div>
    ...
    <section class="info-row page-section" data-belongs-to="system">
        <div class="collapsible-section collapsed" id="advancedStatsSection">
            <div class="collapsible-header" role="button" tabindex="0" aria-expanded="false" aria-controls="advancedStatsContent">
                ...
                <svg class="collapsible-chevron" viewBox="0 0 20 20" aria-hidden="true">
            ...
            <div class="collapsible-content" id="advancedStatsContent">
                <div class="info-card" id="persistentStatsCard">
                    ...
                    <div id="psUnusedWarning" class="info-hint text-warning" style="display: none;"></div>
                </div>
                ...
                <div class="info-card" id="aimdCard">
                    <h3>
                        ...
                        <button class="btn btn-xs ml-8" id="aimdToggleBtn" data-action="toggle-aimd-mode" title="Toggle between observe_only and enforce mode" style="display:none;">Toggle Mode</button>
                    </h3>
    ...
    <div class="page-section request-tab-content" data-belongs-to="requests" data-tab="table">
        <div class="info-card p-12">
            <h3 class="m-0 mb-8 text-heading-sm">Recent Requests</h3>
    ...
    <div class="bottom-drawer" id="bottomDrawer" data-testid="bottom-drawer">
        <div class="drawer-header" data-action="toggle-drawer">
            ...
            <div class="dock-tabs flex ml-8" style="gap:2px;" role="tablist" aria-label="Dock panels">
                <button class="dock-tab dock-tab-base active" id="dock-tab-live" data-dock-tab="live" data-testid="tab-live" data-action="switch-dock-tab" role="tab" aria-selected="true" aria-controls="tab-live" tabindex="0">Live <span class="tab-shortcut">1</span></button>
                <button class="dock-tab dock-tab-base" id="dock-tab-traces" data-dock-tab="traces" data-testid="tab-traces" data-action="switch-dock-tab" role="tab" aria-selected="false" aria-controls="tab-traces" tabindex="-1">Traces <span class="tab-shortcut">2</span></button>
                <button class="dock-tab dock-tab-base" id="dock-tab-logs" data-dock-tab="logs" data-testid="tab-logs" data-action="switch-dock-tab" role="tab" aria-selected="false" aria-controls="tab-logs" tabindex="-1">Logs <span class="tab-shortcut">3</span></button>
                <button class="dock-tab dock-tab-base" id="dock-tab-queue" data-dock-tab="queue" data-testid="tab-queue" data-action="switch-dock-tab" role="tab" aria-selected="false" aria-controls="tab-queue" tabindex="-1">Queue <span class="tab-shortcut">4</span></button>
                <button class="dock-tab dock-tab-base" id="dock-tab-circuit" data-dock-tab="circuit" data-testid="tab-circuit" data-action="switch-dock-tab" role="tab" aria-selected="false" aria-controls="tab-circuit" tabindex="-1">Circuit <span class="tab-shortcut">5</span></button>
            </div>
        ...
        <div class="tab-content">
            <div class="tab-panel active" id="tab-live" role="tabpanel" aria-labelledby="dock-tab-live">
                <!-- Filter Toolbar -->
                <div class="stream-toolbar" data-testid="stream-toolbar">
                    ...
                    <div class="stream-controls">
                        ...
                        <button class="btn btn-icon" id="autoScrollToggle" data-action="toggle-autoscroll" title="Toggle auto-scroll (on)" aria-label="Toggle auto-scroll">
                            <span class="autoscroll-icon">${svgIcon('chevron-down')}</span>
                        </button>
                        <button class="btn btn-icon" data-action="jump-to-latest" title="Jump to latest" aria-label="Jump to latest">
                            <span>${svgIcon('chevron-down')}</span>
                        </button>
                    </div>
                </div>
                <span class="visually-hidden" aria-live="polite" id="liveStreamStatus"></span>
                <div class="virtual-scroll-viewport flex-1-1-auto overflow-y-auto min-h-200 max-h-60vh" data-testid="virtual-viewport" tabindex="0">
                    <div class="virtual-scroll-content request-list" id="liveStreamRequestList" data-testid="virtual-content" role="region" aria-atomic="false" aria-label="Request stream updates">
            ...
            <div class="tab-panel" id="tab-traces" role="tabpanel" aria-labelledby="dock-tab-traces">
                ...
                <div id="traceDetailPanel" class="trace-detail-panel trace-detail-panel--hidden bg-card rounded-md border">
                    <div class="trace-detail-header flex justify-between items-center p-12 border-b">
                        ...
                        <div class="flex gap-6">
                            ...
                            <button class="btn btn-icon btn-small" data-action="close-trace-detail" title="Close" aria-label="Close">${svgIcon('x')}</button>
                        </div>
                    </div>
                    <div class="trace-detail-body p-12">
                        ...
                        <div class="trace-summary grid-auto-fit mb-16">
                            ...
                            <div class="trace-stat">
                                <div class="trace-stat-label">Estimated Cost</div>
                                <div id="traceDetailCost" class="monospace text-sm">-</div>
                            </div>
                        </div>
            ...
            <!-- Tab 3: Logs -->
            <div class="tab-panel" id="tab-logs" role="tabpanel" aria-labelledby="dock-tab-logs">
            ...
            <!-- Tab 4: Queue -->
            <div class="tab-panel" id="tab-queue" role="tabpanel" aria-labelledby="dock-tab-queue">
            ...
            <div class="tab-panel" id="tab-circuit" role="tabpanel" aria-labelledby="dock-tab-circuit">
    ...
    <aside class="side-panel" id="sidePanel" data-testid="side-panel">
        ...
        <div class="detail-section" id="detailTraceSection" style="display:none; padding: 0 16px 16px;">
            <div class="detail-section-title">Timing Waterfall</div>
            <div id="detailTraceWaterfall"></div>
        </div>
    </aside>
    ...
    <script src="/dashboard/js/request-ids.js?v=${ASSET_VERSION}"${nonce ? ` nonce="${nonce}"` : ''}></script>

...
class ModelRouter {
    ...
    _estimateRequestTokens(parsedBody) {
        ...
        // Extended thinking budget (budget_tokens) - Claude Code sends this for reasoning
        // This is a token budget, not characters, so add directly to the final token count
        let thinkingBudgetTokens = 0;
        if (parsedBody.thinking && typeof parsedBody.thinking.budget_tokens === 'number') {
            thinkingBudgetTokens = parsedBody.thinking.budget_tokens;
        }

        const estimatedInputTokens = Math.ceil(inputChars / CHARS_PER_TOKEN) + thinkingBudgetTokens;
    ...
    _matchGlob(pattern, value) {
        let regex = this._globCache.get(pattern);
        if (!regex) {
            // ReDoS protection: limit wildcard count and pattern length
            const wildcardCount = (pattern.match(/\*/g) || []).length;
            if (wildcardCount > 5 || pattern.length > 200) return pattern === value;
            const escaped = pattern
                .replace(/[.+?^${}()|[\]\\]/g, '\\$&')
                .replace(/\*/g, '[^.]*');
            regex = new RegExp(`^${escaped}$`);
            this._globCache.set(pattern, regex);
        }
        ...
        if (classification) {
            ...
            if (targetUnavailable && canSwitch && candidates.length > 1) {
                ...
                if (!foundFallback) {
                    ...
                    // Capture all cooldowns at the same timestamp to avoid time-based flakiness
                    const now = Date.now();
                    const cooldowns = new Map();
                    for (const candidate of candidates) {
                        if (attemptedModels.has(candidate)) continue;
                        const entry = this._cooldowns.get(candidate);
                        if (!entry) {
                            cooldowns.set(candidate, 0);
                        } else {
                            const cooldownConfig = this.config.cooldown || {};
                            const decayMs = cooldownConfig.decayMs || 60000;
                            if (now - entry.lastHit > decayMs) {
                                cooldowns.set(candidate, 0);
                            } else {
                                cooldowns.set(candidate, Math.max(0, entry.cooldownUntil - now));
                            }
                        }
                    }
                    ...
                    for (const candidate of candidates) {
                        if (attemptedModels.has(candidate)) continue;
                        if (!cooldowns.has(candidate)) continue;
                        const cd = cooldowns.get(candidate);
                        // Context window pre-flight
                        if (estimatedTokens > 0) {
     ...
     *     recent429 (number), latencyP95 (null — per-key only), errorRate (number), cost
     */
    async getModelPoolSnapshot() {
        ...
        for (const [tier, cfg] of Object.entries(this.config.tiers || {})) {
            ...
            pools[tier] = await Promise.all(candidates.map(async model => {
                ...
                // Wire recent429 from _recentPool429s sliding window
                const recent429Count = this.getPool429Count(model);

                // NOTE: latencyP95 is tracked per-key in KeyManager, not per-model
                // in the router. Wiring this would require a KeyManager reference
                // and model-to-key mapping which is outside this snapshot's scope.
                const latencyP95 = null;

                // Compute errorRate from recent 429s vs total selections
                const totalSelections = this._stats.byModel[model] || 0;
                let errorRate = null;
                if (totalSelections > 0) {
                    errorRate = Math.min(1, recent429Count / totalSelections);
                } else if (recent429Count > 0) {
                    // 429s recorded but no selections tracked — assume 100% error
                    errorRate = 1;
                } else {
                    errorRate = 0;
                }
    ...
    /**
     * Hot-update the routing config without losing cooldown or override state.
     * Clears glob pattern cache and re-validates tier overlaps.
     *
     * @param {Object} config - New model routing config block
     * @returns {void}
     */
    updateConfig(config) {
    ...
    /**
     * Return a full state snapshot suitable for JSON serialization.
     * Outputs normalized v2 config format with models[] arrays.
     *
     * @returns {Object} State snapshot with shadowMode, enabled, config, stats, and migration flag
     */
    toJSON() {
    ...
    static validateConfig(updates) {
        ...
        const hasCatchAll = hasRules &&
            updates.rules.some((rule) => {
                if (!rule.match || typeof rule.match !== 'object') return false;
                // Explicit wildcard: { model: '*' }
                if (rule.match.model === '*') return true;
                // Empty match object {} matches everything (catch-all)
                if (Object.keys(rule.match).length === 0) return true;
                return false;
            });

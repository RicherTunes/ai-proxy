...
/**
 * Maximum allowed length for x-model-override header values.
 * @constant {number}
 */
const MAX_OVERRIDE_LENGTH = 128;

/**
 * Sanitize a model override value: strip control characters and enforce length limit.
 * @param {string} value - Raw override header value
 * @returns {string|null} Sanitized value, or null if empty after sanitization
 */
function sanitizeModelOverride(value) {
    if (typeof value !== 'string') return null;
    // Strip control characters (NUL, newlines, tabs, etc.)
    let sanitized = value.replace(/[\x00-\x1f\x7f]/g, '');
    // Enforce length limit
    if (sanitized.length > MAX_OVERRIDE_LENGTH) {
        sanitized = sanitized.slice(0, MAX_OVERRIDE_LENGTH);
    }
...
async function transformRequestBody(
    ...
    if (modelRouter) {
        ...
        if (req && req.headers && req.headers['x-model-override']) {
            const requestedOverride = sanitizeModelOverride(req.headers['x-model-override']);

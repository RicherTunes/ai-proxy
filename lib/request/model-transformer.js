...
async function transformRequestBody(
    ...
    // Resolve provider for the original model (no routing decision)
    const resolvedProvider = providerRegistry
        ? providerRegistry.resolveProviderForModel(originalModel, null)
        : null;
    const providerName = resolvedProvider ? resolvedProvider.providerName : null;

    // No routing decision — still inject stream_options for cost tracking
    if (parsed.stream === true && !parsed.stream_options) {
        parsed.stream_options = { include_usage: true };
        const newBody = Buffer.from(JSON.stringify(parsed), 'utf8');
        return { body: newBody, originalModel, mappedModel: originalModel, routingDecision: null, provider: providerName };
    }

    return { body, originalModel, mappedModel: originalModel, routingDecision: null, provider: providerName };
}

...
async function transformRequestBody(
    ...
    if (modelRouter) {
        ...
        if (routingResult && routingResult.model) {
            ...
            // Inject stream_options for streaming requests so z.ai returns usage data
            // (also done unconditionally below for non-routed requests)
            ...
            // Resolve provider for the routed model.
            // TODO(Month 7): Pass actual modelMapping config instead of null
            // so provider-specific model entries (e.g. { target, provider }) are consulted.
            // Currently always resolves to the default provider.
            const resolvedProvider = providerRegistry
                ? providerRegistry.resolveProviderForModel(routingResult.model, null)
                : null;
    ...
    // TODO(Month 7): Pass modelMapping so provider-specific entries are consulted.
    const noRouteProvider = providerRegistry
        ? providerRegistry.resolveProviderForModel(originalModel, null)
        : null;

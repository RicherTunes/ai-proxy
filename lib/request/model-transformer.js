...
async function transformRequestBody(
    ...
    // Handle empty body
    if (body.length === 0) {
        return { body, originalModel: null, mappedModel: null, routingDecision: null };
    }
    ...
    if (parsed.stream === true && !parsed.stream_options) {
        parsed.stream_options = { include_usage: true };
        const newBody = Buffer.from(JSON.stringify(parsed), 'utf8');
        return { body: newBody, originalModel, mappedModel: originalModel, routingDecision: null };
    }

    return { body, originalModel, mappedModel: originalModel, routingDecision: null };
}

...
class CostTracker {
    ...
    _recordCostTimeSeries(cost, model) {
        ...
        if (lastIdx >= 0 && ts.times[lastIdx] === hourKey) {
            ...
            // Cap tracked models at 100 to prevent unbounded growth
            if (!ts.byModel[model] && Object.keys(ts.byModel).length >= 100) {
                return; // Skip per-model tracking for new models past the cap
            }
            ...
            // Cap tracked models at 100 to prevent unbounded growth
            if (!ts.byModel[model] && Object.keys(ts.byModel).length < 100) {
                ts.byModel[model] = new Array(ts.times.length).fill(0);
            }
            if (ts.byModel[model]) {
    ...
    getStats(period = 'today') {
        ...
        return {
            ...
            avgCostPerRequest: usageData.requests > 0
                ...
                : 0,
            costPer1kTokens: usageData.totalTokens > 0
                ? Math.round((usageData.cost / (usageData.totalTokens / 1000)) * 1000000) / 1000000
                : 0,

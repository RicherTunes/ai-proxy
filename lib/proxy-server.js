...
const { UpstreamHealthMonitor } = require('./upstream-health');
...
// Deferred module-load warnings: collected at require()-time (before logger exists),
// flushed to structured logger in ProxyServer constructor via _flushDeferredWarnings().
const _deferredLoadWarnings = [];
...
try {
    ...
    if (e.code !== 'MODULE_NOT_FOUND') _deferredLoadWarnings.push({ module: 'route-policy', error: e.message });
}
...
try {
    ...
    if (e.code !== 'MODULE_NOT_FOUND') _deferredLoadWarnings.push({ module: 'predictive-scaler', error: e.message });
}
...
try {
    ...
    ReplayQueue = null;
    if (e.code !== 'MODULE_NOT_FOUND') _deferredLoadWarnings.push({ module: 'replay-queue', error: e.message });
}
...
try {
    ...
    PluginManager = null;
    if (e.code !== 'MODULE_NOT_FOUND') _deferredLoadWarnings.push({ module: 'plugin-manager', error: e.message });
}

// SSE keepalive interval (ms) — sent as comment frames to prevent idle timeouts
...
class ProxyServer {
    constructor(options = {}) {
        ...
        // Flush deferred module-load warnings now that logger is available
        for (const warning of _deferredLoadWarnings) {
            this.logger.warn('Failed to load optional module', { module: warning.module, error: warning.error });
        }
        _deferredLoadWarnings.length = 0;
        ...
        this.keyManager = new KeyManager({
            ...
            defaultProviderName: this.config._providerRegistry?.defaultProviderName || 'z.ai',
        ...
        if (this.config.usageMonitor?.enabled !== false
            ...
            this.usageMonitor.setAnomalyCallback((alert) => this._handleAnomalyAlert(alert));
        }

        // Rate Limit Sync — dynamically updates per-model concurrency baselines (primary only)
        this.rateLimitSync = null;
        if (this.config.rateLimitSync?.enabled !== false && !this.isClusterWorker) {
            const { RateLimitSync } = require('./rate-limit-sync');
            this.rateLimitSync = new RateLimitSync(
                this.config.rateLimitSync || {},
                {
                    logger: this.logger.child ? this.logger.child('rate-limit-sync') : this.logger,
                    keyManager: this.keyManager,
        ...
        // Wire rate limit sync to request handler for header feeding
        if (this.rateLimitSync) {
            this.requestHandler.rateLimitSync = this.rateLimitSync;
        }
        ...
        // Subscribe to request-start and request-retry events for live Runway visualization
        this.requestHandler.on('request-start', (data) => this._broadcastSSE('request-start', data));
        this.requestHandler.on('request-retry', (data) => this._broadcastSSE('request-retry', data));
        ...
        if (this.config.adminAuth?.enabled) {
            this.adminAuth = new AdminAuth({
                ...
                sessionCookieName: this.config.adminAuth.sessionCookieName,
                sessionTtlMs: this.config.adminAuth.sessionTtlMs,
        ...
        // Note: modelDiscovery config is nested under security in DEFAULT_CONFIG
        const mdConfig = this.config.security?.modelDiscovery || {};
        this.modelDiscovery = new ModelDiscovery({
            cacheTTL: mdConfig.ttl || 5 * 60 * 1000,
            ...
            customModels: mdConfig.customModels || [],
            probeDelayMs: mdConfig.probeDelayMs,
            probeTimeoutMs: mdConfig.probeTimeoutMs,
            persistFile: mdConfig.persistFile,
            logger: this.logger.child ? this.logger.child('model-discovery') : this.logger
        });

        // Late-bind modelDiscovery into rateLimitSync (constructed earlier in init sequence)
        if (this.rateLimitSync) {
            this.rateLimitSync._modelDiscovery = this.modelDiscovery;
        }

        // Subscribe to model probe events for live dashboard visibility
        this.modelDiscovery.on('probe-status', (data) => this._broadcastSSE('probe-status', data));
        ...
        // Start rate limit sync (header observation + ceiling probe)
        if (this.rateLimitSync) {
            this.rateLimitSync.start();
            this.logger.info('Rate limit sync started');
        }
        ...
        this.upstreamHealth = new UpstreamHealthMonitor({
            primaryHost: this.config.targetHost,
            primaryPort: 443,
            primaryBasePath: this.config.targetBasePath,
            primaryProtocol: this.config.targetProtocol,
            fallbacks: this.config.upstreamFallbacks || [],
            probeIntervalMs: 15000,
            logger: this.logger.child('upstream-health')
        });

        if (!this.config.upstreamFallbacks || this.config.upstreamFallbacks.length === 0) {
            this.logger.warn('No upstream fallback endpoints configured — failover is disabled. Set upstreamFallbacks in config to enable.');
        }

        // Broadcast health events via SSE
        this.upstreamHealth.on('upstream-health', (data) => this._broadcastSSE('upstream-health', data));
        this.upstreamHealth.on('upstream-failover', (data) => {
            this._broadcastSSE('upstream-failover', data);
            // Update request handler's target dynamically
            const target = this.upstreamHealth.getActiveTarget();
            if (this.requestHandler) {
                this.requestHandler.targetHost = target.host;
                this.requestHandler.targetBasePath = target.basePath;
                this.requestHandler.targetProtocol = target.protocol;
                this.requestHandler.useHttps = target.protocol === 'https:';
                this.requestHandler._createAgent();  // Recreate agent for new host
                this.logger.warn('Request handler target updated for failover', target);
            }
        });
        this.upstreamHealth.on('upstream-restored', (data) => {
            this._broadcastSSE('upstream-restored', data);
            // Restore original target
            if (this.requestHandler) {
                this.requestHandler.targetHost = this.config.targetHost;
                this.requestHandler.targetBasePath = this.config.targetBasePath;
                this.requestHandler.targetProtocol = this.config.targetProtocol;
                this.requestHandler.useHttps = this.config.targetProtocol === 'https:';
                this.requestHandler._createAgent();
                this.logger.info('Request handler target restored to primary');
            }
        });

        this.upstreamHealth.start();
    ...
    _reloadKeys() {
        try {
            ...
            // Re-read persisted model routing config from disk (not from hardcoded defaults)
            // Bug fix: this.config.modelRouting contains defaults from config.js, NOT the user's
            // saved model-routing.json. Using defaults here would erase the user's tier customizations.
            if (this.modelRouter && this._routingPersistence?.configPath) {
                try {
                    const persisted = fs.readFileSync(this._routingPersistence.configPath, 'utf8');
                    const parsed = JSON.parse(persisted);
                    this.modelRouter.updateConfig(parsed);
                } catch (_e) {
                    // No persisted config or parse error — keep current runtime config as-is
                }
            }
    ...
    _isAdminRoute(path) {
        ...
        if (ProxyServer._ADMIN_EXACT_PATHS.has(path)) return true;
        // Check prefix matches
        for (const prefix of ProxyServer._ADMIN_PREFIX_PATHS) {
    ...
    _requireAuth(req, res) {
        const authResult = this.adminAuth.authenticate(req, { forceRequired: true });
    ...
    _routeRequest(req, res) {
        if (this.isShuttingDown) {
            this._sendError(res, 503, 'Server is shutting down');
            return;
        }
        ...
        switch (url) {
            case '/auth-status':
                ...
                break;
            case '/auth/login':
                this._handleAuthLogin(req, res);
                break;
            case '/auth/logout':
                this._handleAuthLogout(req, res);
                break;
            case '/health':
            ...
            case '/models':
                ...
                break;
            case '/models/refresh':
                this._safeAsync(this._handleModelsRefresh(req, res), req, res);
                break;
            case '/replay-queue':
            ...
            case '/upstream-health':
                ...
                res.end(JSON.stringify(this.upstreamHealth.getStatus()));
                break;
            case '/adaptive-concurrency':
                this._safeAsync(this._handleAdaptiveConcurrency(req, res), req, res);
                break;
            default:
                // Dynamic routes
    ...
    _handleAuthStatus(req, res) {
        ...
        if (this.adminAuth && this.adminAuth.enabled) {
            ...
            authenticated = this.adminAuth.peekAuthentication(req).authenticated;
        }
    ...
    _handleAuthLogin(req, res) {
        ...
        if (!this.adminAuth || !this.adminAuth.enabled) {
            ...
            res.end(JSON.stringify({ error: 'auth_disabled' }));
            return;
        }

        const authResult = this.adminAuth.createSessionFromRequest(req);
        if (!authResult.authenticated) {
            const status = authResult.error === 'too_many_attempts' ? 429 : 401;
            const headers = { 'content-type': 'application/json' };
            if (authResult.retryAfterMs) {
    ...
    _handleAuthLogout(req, res) {
        ...
        if (this.adminAuth) {
            this.adminAuth.clearSession(req);
        }

        res.writeHead(200, {
            'content-type': 'application/json',
            'set-cookie': this.adminAuth ? this.adminAuth.clearSessionCookie(req) : 'glm_admin_session=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT'
        });
    ...
    _handleStats(req, res) {
        ...
        // Per-provider key health (Month 9 observability)
        stats.providerHealth = this.keyManager.getProviderHealthStats?.() || {};

        // z.ai account usage (single source of truth: UsageMonitor)
        if (this.usageMonitor) {
            stats.accountUsage = this.usageMonitor.getSnapshot();
            const anomalyAlerts = this.usageMonitor.getAnomalyAlerts();
            if (anomalyAlerts.length > 0) {
                stats.accountUsage.anomalyAlerts = anomalyAlerts;
            }
        }

        // Dynamic rate limit sync observations (header-observed + ceiling probe)
        if (this.rateLimitSync) {
            stats.rateLimitSync = this.rateLimitSync.getSnapshot();
        }

        // Upstream health monitor status
        stats.upstreamHealth = this.upstreamHealth?.getStatus() || null;
    ...
    _handleDashboardAsset(req, res, url) {
        const ASSET_WHITELIST = {
            // Phase 5: Modular CSS files (split from legacy dashboard.css monolith)
            ...
            '/dashboard/js/request-ids.js': { file: 'js/request-ids.js', contentType: 'application/javascript; charset=utf-8' },
    ...
    _checkRateLimit(ip, type = 'api') {
        ...
        // Hard cap: evict expired entries when map grows too large (prevent memory leak)
        if (this._rateLimitMap.size > 50000) {
            for (const [k, v] of this._rateLimitMap) {
                if (v.resetTime < now) this._rateLimitMap.delete(k);
            }
            // If still over cap after eviction, drop oldest half
            if (this._rateLimitMap.size > 50000) {
                let toDelete = Math.floor(this._rateLimitMap.size / 2);
                for (const k of this._rateLimitMap.keys()) {
                    if (toDelete-- <= 0) break;
                    this._rateLimitMap.delete(k);
                }
            }
        }
    ...
    _handleRequestStream(req, res) {
        ...
        // Send keepalive comments every SSE_KEEPALIVE_INTERVAL_MS
     ...
     * Used for request-start and request-retry events (Runway visualization)
     */
    _broadcastSSE(eventName, data) {
        const payload = `event: ${eventName}\ndata: ${JSON.stringify(data)}\n\n`;

        // /requests/stream clients (dashboard)
        for (const client of this.sseStreamClients) {
            try {
                if (client.res.writableEnded) continue;
        ...
        // /events clients (E2E + dashboards)
        if (this.sseEventClients.size > 0) {
            const eventPayload = `event: ${eventName}\ndata: ${JSON.stringify({ ...data, type: eventName, ts: Date.now(), schemaVersion: 1 })}\n\n`;
            for (const client of this.sseEventClients) {
                try {
                    if (client.res.writableEnded) continue;
        ...
        try {
            ...
            if (req.method === 'GET') {
                ...
                if (updates.rates) {
                    ...
                    this._addAuditEntry('cost_tracking_rates_updated', {
                        ip,
                        changes: { inputTokenPer1M: updates.rates.inputTokenPer1M, outputTokenPer1M: updates.rates.outputTokenPer1M },
                        timestamp: new Date().toISOString()
                    });
                }
                ...
                if (updates.modelRates) {
                    ...
                    this._addAuditEntry('cost_tracking_model_rates_updated', {
                        ip, models: Object.keys(updates.modelRates), timestamp: new Date().toISOString()
                    });
                }
                ...
                if (updates.budget) {
                    ...
                    this._addAuditEntry('cost_tracking_budget_updated', {
                        ip,
                        changes: { daily: updates.budget.daily, monthly: updates.budget.monthly, alertThresholds: updates.budget.alertThresholds },
                        timestamp: new Date().toISOString()
                    });
                }
                ...
                if (updates.saveDebounceMs !== undefined) {
                    ...
                    this._addAuditEntry('cost_tracking_config_updated', {
                        ip, changes: { saveDebounceMs: updates.saveDebounceMs }, timestamp: new Date().toISOString()
                    });
                }
    ...
    /**
     * Handle /adaptive-concurrency endpoint
     * GET: Returns current AIMD snapshot (mode, per-model windows)
     * PUT: Toggle mode between 'observe_only' and 'enforce'
     */
    async _handleAdaptiveConcurrency(req, res) {
        if (!this.adaptiveConcurrency) {
            this._sendError(res, 503, 'Adaptive concurrency not enabled');
            return;
        }

        if (req.method === 'GET') {
            const snapshot = this.adaptiveConcurrency.getSnapshot();
            this._sendJson(res, 200, snapshot);
            return;
        }

        if (req.method === 'PUT') {
            const data = await parseJsonBody(req, { allowEmpty: false });
            const newMode = data.mode;

            let result;
            try {
                result = this.adaptiveConcurrency.setMode(newMode);
            } catch (e) {
                this._sendError(res, 400, e.message);
                return;
            }
            ...
            this._sendJson(res, 200, {
                ...
                snapshot: this.adaptiveConcurrency.getSnapshot()
            });
            return;
        }
    ...
    /**
     * Handle /models/refresh endpoint
     * GET: Return current probe state
     * POST: Trigger model probe against z.ai
     */
    async _handleModelsRefresh(req, res) {
        if (req.method === 'GET') {
            this._sendJson(res, 200, this.modelDiscovery.getProbeState());
            return;
        }

        if (req.method !== 'POST') {
            this._sendError(res, 405, 'Method not allowed, use GET or POST');
            return;
        }

        // Parse request body (using centralized parser with size limit)
        let body = {};
        try {
            body = await parseJsonBody(req, { allowEmpty: true });
        } catch (e) {
            this._sendError(res, e.statusCode || 400, e.message || 'Invalid JSON body');
            return;
        }

        // Get an API key for probing
        let apiKey;
        try {
            const keyInfo = this.keyManager.getKey ? this.keyManager.getKey() : this.keyManager.acquireKey?.();
            apiKey = keyInfo?.key || keyInfo;
            if (!apiKey && this.config.apiKeys?.length > 0) {
                apiKey = this.config.apiKeys[0];
            }
        } catch {
            // Fallback to first configured key
            apiKey = this.config.apiKeys?.[0];
        }

        if (!apiKey) {
            this._sendError(res, 503, 'No API keys available for probing');
            return;
        }

        const result = await this.modelDiscovery.probeModels({
            apiKey,
            targetHost: this.config.targetHost || 'api.z.ai',
            targetBasePath: this.config.targetBasePath || '/api/anthropic',
            targetProtocol: this.config.targetProtocol || 'https:',
            probeKnown: body.probeKnown !== false,
            probeCandidates: body.probeCandidates !== false,
            userCandidates: Array.isArray(body.userCandidates) ? body.userCandidates : []
        });

        if (result.error === 'probe_already_running') {
            this._sendJson(res, 409, { error: 'Probe already in progress', state: this.modelDiscovery.getProbeState() });
            return;
        }

        if (result.error) {
            this._sendError(res, 500, result.message || result.error);
            return;
        }
    ...
    async _handleModelRoutingEnableSafe(req, res) {
        ...
        try {
            ...
            // Add default tier configs if requested — but ONLY if the router doesn't already have them.
            // Existing user-customized tiers (v2 multi-model) must never be overwritten with v1 defaults.
            if (addDefaultRules) {
                const existingTiers = this.modelRouter.config?.tiers || {};
                if (!updates.tiers) {
                    updates.tiers = {};
                }
                // Preserve existing tiers: only fill in missing ones with defaults
                if (!updates.tiers.light && !existingTiers.light) {
                    updates.tiers.light = { targetModel: 'glm-4.5-air', clientModelPolicy: 'rule-match-only' };
                }
                if (!updates.tiers.medium && !existingTiers.medium) {
                    updates.tiers.medium = { targetModel: 'glm-4.6', clientModelPolicy: 'rule-match-only' };
                }
                if (!updates.tiers.heavy && !existingTiers.heavy) {
            ...
            // Deep-merge tiers to preserve existing user config when only adding missing defaults
            // Shallow spread would replace all tiers if updates.tiers exists
            const mergedConfig = { ...this.modelRouter.config, ...updates };
            if (updates.tiers && this.modelRouter.config?.tiers) {
                mergedConfig.tiers = { ...this.modelRouter.config.tiers, ...updates.tiers };
            }
    ...
    _handleAnomalyAlert(alert) {
        const level = alert.severity === 'critical' ? 'error' : 'warn';
        this.logger[level]('Usage anomaly detected', alert);
        if (this.webhookManager) {
            this.webhookManager.emit(alert.type, alert.data || alert, { dedupeKey: `anomaly_${alert.type}` });
        }
    }
    ...
    async _handleControl(req, res, pathname) {
        ...
        try {
            ...
            if (pathname === '/control/restart') {
                this.logger.info('Graceful restart requested via API');
                const restartCluster = require('cluster');
                const isCluster = this.isClusterWorker ||
                    (restartCluster.isPrimary && restartCluster.workers && Object.keys(restartCluster.workers).length > 0);
                const mode = isCluster ? 'rolling' : 'full';
                auditControl('restart', { mode });
                sendJson(200, { status: 'restarting', message: 'Graceful restart initiated', mode });
                // Delay restart slightly to allow response to be sent
                setTimeout(() => this._performRestart(), 100);
                return;
            }
    ...
    async start() {
        ...
        return new Promise((resolve, reject) => {
            this.server.listen(this.config.port, this.config.host, () => {
                ...
                this.logger.info(`/requests/stream (SSE) | /circuit-history`);

                // Fire-and-forget startup probe (non-blocking)
                this._probeOnStartup();
    ...
    /**
     * Probe all known + candidate models on startup (non-blocking).
     * Runs only when modelDiscovery.probeOnStartup is enabled and we have an API key.
     */
    _probeOnStartup() {
        if (!this.config.security?.modelDiscovery?.probeOnStartup) return;
        if (!this.modelDiscovery) return;

        // Mirror the /models/refresh endpoint's key extraction pattern
        const keyInfo = this.keyManager?.acquireKey?.();
        const apiKey = keyInfo?.key || keyInfo
            || (this.config.apiKeys && this.config.apiKeys[0]);
        if (!apiKey) {
            this.logger.warn('Startup probe skipped: no API key available');
            return;
        }

        this.logger.info('Starting model probe on startup...');
        this.modelDiscovery.probeModels({
            apiKey,
            ...
            if (result.error) {
                this.logger.warn('Startup probe failed', { error: result.error });
                return;
            }
            this.logger.info('Startup probe complete', {
                verified: result.results?.verified?.length || 0,
                discovered: result.results?.discovered?.length || 0,
                duration: result.duration
            });
        }).catch(err => {
            this.logger.warn('Startup probe error', { error: err.message });
        });
    }

    /**
     * Perform graceful restart.
     * Cluster worker: signal master for rolling restart, then exit (master respawns).
     * Single-process: shutdown and exit (PM2 or self-restart).
     */
    async _performRestart() {
        const cluster = require('cluster');

        if (cluster.isWorker) {
            // Tell master to do a rolling restart of all workers
            process.send({ type: 'restart_requested', workerId: cluster.worker.id });
            return;
        }
        ...
        if (isPM2) {
            this.logger.info('PM2 detected — shutting down for PM2 auto-restart');
            await this.shutdown();
            process.exit(0);
        } else {
            // Self-restart: spawn replacement, then exit
            const { spawn } = require('child_process');
            this.logger.info('Self-restart: spawning replacement process');
    ...
    async shutdown() {
        ...
        // 3.5d. Persist cache and stop rate limit sync
        if (this.rateLimitSync) {
            await this.rateLimitSync.persistAndStop();
        }
        ...
        // 3.9. Stop upstream health monitor
        if (this.upstreamHealth) this.upstreamHealth.stop();
        ...
        if (this.adminAuth && typeof this.adminAuth.destroy === 'function') {
...
// Static admin route lookup tables (built once, used per-request)
ProxyServer._ADMIN_EXACT_PATHS = new Set([
    '/health', '/health/deep', '/metrics', '/stats', '/persistent-stats',
    '/reload', '/backpressure', '/logs', '/history', '/dashboard',
    '/requests/stream', '/requests', '/circuit-history',
    '/stats/account-details', '/stats/latency-histogram', '/stats/cost',
    '/stats/cost/history', '/stats/scheduler', '/stats/tenants',
    '/traces', '/predictions', '/compare', '/webhooks', '/tenants',
    '/model-mapping', '/model-routing', '/model-routing/export',
    '/model-routing/import-from-mappings', '/model-routing/enable-safe',
    '/model-routing/explain', '/model-routing/simulate', '/model-selection',
    '/models', '/models/refresh', '/replay-queue', '/plugins',
    '/auth-status', '/auth/login', '/auth/logout', '/events',
    '/admin/cost-tracking', '/admin/cost-tracking/config',
    '/admin/cost-tracking/metrics', '/admin/cost-tracking/flush',
    '/admin/cost-tracking/reset', '/adaptive-concurrency', '/upstream-health'
]);
ProxyServer._ADMIN_PREFIX_PATHS = [
    '/debug/', '/control/', '/dashboard/', '/traces/', '/policies/',
    '/model-routing/', '/model-routing/export/', '/replay/', '/replay-queue/',
    '/plugins/', '/admin/cost-tracking/', '/requests/', '/stats/latency-histogram/'
];
...
async function startMaster(config, numWorkers) {
    ...
    const handleWorkerMessage = (msg) => {
        ...
        } else if (msg.type === 'restart_requested') {
            logger.info('Rolling restart requested by worker ' + msg.workerId);
            rollingRestart();
        }
    };

    // Rolling restart: restart workers one-by-one for zero-downtime
    let rollingRestartInProgress = false;

    async function rollingRestart() {
        if (rollingRestartInProgress) {
            logger.warn('Rolling restart already in progress, ignoring');
            return;
        }
        rollingRestartInProgress = true;

        const workerIds = Object.keys(cluster.workers);
        logger.info(`Rolling restart: ${workerIds.length} workers to restart`);

        for (const id of workerIds) {
            const worker = cluster.workers[id];
            if (!worker) continue;

            logger.info(`Restarting worker ${id} (PID: ${worker.process.pid})`);
            worker.send({ type: 'graceful_shutdown' });
            worker.disconnect();

            // Wait for worker to exit (master's 'exit' handler respawns it)
            await new Promise(resolve => {
                const onExit = () => {
                    // Stagger: give the new worker time to start
    ...
    // Fork-bomb guard: track worker deaths to prevent crash loops
    const workerDeaths = [];

    // Handle worker crashes
    cluster.on('exit', (worker, code, signal) => {
        // Track death timestamp
        workerDeaths.push(Date.now());

        // Prune deaths older than 30 seconds to prevent unbounded growth
        const now = Date.now();
        while (workerDeaths.length > 0 && now - workerDeaths[0] > 30000) {
            workerDeaths.shift();
        }

        // Check for crash loop (more than 5 deaths in 30 seconds)
        // Skip during intentional rolling restart
        if (!rollingRestartInProgress && workerDeaths.length > 5) {
            logger.error(`Worker crash loop detected (${workerDeaths.length} deaths in 30s). Halting respawns.`);
            return;
        }
...
async function startWorker(config, isCluster) {
    ...
    // Handle rolling restart message from master
    process.on('message', (msg) => {
        if (msg && msg.type === 'graceful_shutdown') {
            proxy.logger.info('Graceful shutdown requested by master (rolling restart)');
            shutdown();
        }
    });

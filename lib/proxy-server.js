    ...
    async _handleAdaptiveConcurrency(req, res) {
        ...
        if (req.method === 'PUT') {
            ...
            if (newMode !== 'observe_only' && newMode !== 'enforce') {
                this._sendError(res, 400, 'mode must be "observe_only" or "enforce"');
                return;
            }

            const oldMode = this.adaptiveConcurrency.config.mode;
            this.adaptiveConcurrency.config.mode = newMode;

            // When switching to enforce, push current windows to keyManager
            if (newMode === 'enforce' && oldMode !== 'enforce') {
                const snapshot = this.adaptiveConcurrency.getSnapshot();
                for (const [model, info] of Object.entries(snapshot.models)) {
                    this.keyManager.setEffectiveModelLimit?.(model, info.effectiveMax);
                }
            }

            // When switching to observe_only, restore static limits
            if (newMode === 'observe_only' && oldMode === 'enforce') {
                this.keyManager.restoreStaticLimits?.();
            }

            this.logger.info('Adaptive concurrency mode changed', { from: oldMode, to: newMode });
            this._sendJson(res, 200, {
                success: true,

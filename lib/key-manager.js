...
class KeyManager {
    ...
    /**
     * Initialize keys from array
     */
    loadKeys(keyStrings) {
        this.keys = keyStrings.map((key, index) => {
            const keyId = KeyManager.getKeyId(key);
            const keyInfo = {
                index,
                key,
                keyId,
     ...
     */
    reloadKeys(keyStrings) {
        const existingKeyIds = new Set(this.keys.map(k => k.keyId));
        const newKeyIds = new Set(keyStrings.map(k => KeyManager.getKeyId(k)));

        // Find removed keys
        const removedKeys = this.keys.filter(k => !newKeyIds.has(k.keyId));

        // Find added keys
        const addedKeyStrings = keyStrings.filter(k => !existingKeyIds.has(KeyManager.getKeyId(k)));
        ...
        keyStrings.forEach((key, index) => {
            ...
            if (existing) {
                // Preserve existing key info, update index and key string (for secret rotation)
                existing.index = index;
                existing.key = key;  // Update key string in case secret portion changed
    ...
    acquireKey(excludeIndices = []) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            const keyInfo = this.getBestKey([...tried]);

...
class KeyManager {
    constructor(options = {}) {
        ...
        this.strictKeyUniqueness = options.strictKeyUniqueness ?? false; // Fail-closed on duplicate keys
    ...
    /**
     * Initialize API keys from a flat array or provider map.
     * Clears all existing keys and initializes the key pool with the provided keys.
     * Each key is assigned an index, wrapped with usage tracking, and initialized with
     * circuit breaker and rate limiting state.
     *
     * @param {string[]|Object.<string, string[]>} keyInput - Either:
     *   - Flat array of key strings (e.g., ['key1.s1', 'key2.s2'])
     *   - Provider map mapping provider names to key arrays (e.g., { 'z.ai': ['key1.s1'], 'anthropic': ['sk-ant.s1'] })
     * @throws {Error} If strictKeyUniqueness is true and duplicate keys are detected across providers
     * @returns {void}
     */
    loadKeys(keyInput) {
        ...
        // Detect duplicate key strings across providers
        const seenKeys = new Map(); // keyId -> provider
        const deduped = [];
        for (const entry of keyEntries) {
            const keyId = KeyManager.getKeyId(entry.key);
            const existing = seenKeys.get(keyId);
            if (existing !== undefined) {
                if (this.strictKeyUniqueness) {
                    throw new Error(
                        `Duplicate key '${keyId.substring(0, 8)}...' found in providers '${existing}' and '${entry.provider}'. ` +
                        `Set strictKeyUniqueness: false to skip duplicates with a warning.`
                    );
                }
                this._log('warn', `Duplicate key '${keyId.substring(0, 8)}...' in providers '${existing}' and '${entry.provider}', skipping duplicate`, {
                    keyId: keyId.substring(0, 8),
                    firstProvider: existing,
                    duplicateProvider: entry.provider
                });
                continue;
            }
            seenKeys.set(keyId, entry.provider);
            deduped.push(entry);
        }

        this.keys = deduped.map(({ key: keyStr, provider }, index) => {
    ...
    /**
     * Hot-reload API keys, preserving statistics for keys that already exist.
     * Compares the new key set against the current one: existing keys retain their
     * usage counters, latency stats, and circuit breaker state; new keys are
     * initialized fresh; removed keys are cleaned up.
     *
     * @param {string[]|Object.<string, string[]>} keyInput - Either:
     *   - Flat array of key strings (e.g., ['key1.s1', 'key2.s2'])
     *   - Provider map mapping provider names to key arrays (e.g., { 'z.ai': ['key1.s1'], 'anthropic': ['sk-ant.s1'] })
    ...
    _getOrCreatePool(model) {
        ...
        if (!this._modelPools.has(key)) {
            // Prevent unbounded growth from arbitrary model names
            if (this._modelPools.size >= 500) return null;
    ...
    recordPoolRateLimitHit(options = {}) {
        ...
        if (!pool) return { cooldownMs: 0, pool429Count: 0, cooldownUntil: 0, model: model || 'global', wasAlreadyBlocked: false };
    ...
    /**
     * Release a key by decrementing its in-flight request counter.
     * Used for late responses where the caller already handled the
     * result recording separately.
     *
     * @param {Object} keyInfo - The key info object from the key pool
     * @param {number} keyInfo.inFlight - The current in-flight count (will be decremented, floored at 0)
     * @returns {void}
     */
    releaseKey(keyInfo) {
        keyInfo.inFlight = Math.max(0, keyInfo.inFlight - 1);
    ...
    /**
     * Get a key info object by its zero-based index in the key pool.
     *
     * @param {number} index - The zero-based index of the key
     * @returns {Object|undefined} The key info object, or undefined if index is out of bounds
     */
    getKeyByIndex(index) {
    ...
    /**
     * Get a key info object by its key ID (the portion before the first dot).
     * Key IDs are extracted using the static getKeyId() method.
     *
     * @param {string} keyId - The key identifier (e.g., 'sk-ant-xxx' from 'sk-ant-xxx.s1')
     * @returns {Object|undefined} The key info object, or undefined if keyId is not found
     */
    getKeyById(keyId) {
    ...
    /**
     * Get health, latency, rate-limit, and circuit-breaker statistics for all keys.
     * Updates the pool-average latency before computing, so health scores reflect
     * the latest data.
     *
     * @returns {Object[]} Array of per-key stat objects, each containing:
     *   - {number} index - Key position in the pool
     *   - {string} keyId - Key identifier
     *   - {string} keyPrefix - First 8 chars of keyId
     *   - {number} inFlight - Currently in-flight request count
     *   - {number} totalRequests - Lifetime request count
     *   - {number} successCount - Lifetime success count
     *   - {number|null} successRate - Percentage (1 decimal), or null if no completed requests
     *   - {Object} latency - { avg, min, max, p50, p95, p99, samples }
     *   - {Object} healthScore - { total, latency, success, errorRecency, isSlowKey }
     *   - {number} lastUsed - Timestamp of last use
     *   - {number} lastSuccess - Timestamp of last success
     *   - {Object} circuitBreaker - Circuit breaker stats
     *   - {Object} rateLimit - Per-key rate limit stats from the rate limiter
     *   - {Object} rateLimitTracking - { count, lastHit, cooldownMs, inCooldown, cooldownRemaining }
     */
    getStats() {
    ...
    /**
     * Update the static baseline concurrency limit for a model.
     * Used by RateLimitSync when upstream rate limits change.
     * Only raises effective limit if it was at or above the old ceiling
     * (preserves AIMD-reduced effective limits).
     * @param {string} model - Model name
     * @param {number} newStaticLimit - New static baseline (>= 1)
     * @returns {{ oldStatic: number|undefined, newStatic: number, effective: number|undefined }}
     */
    updateStaticModelLimit(model, newStaticLimit) {
        if (!model || typeof newStaticLimit !== 'number' || newStaticLimit < 1) {
            return { oldStatic: undefined, newStatic: newStaticLimit, effective: undefined };
        }
        newStaticLimit = Math.floor(newStaticLimit);
        const oldStatic = this._modelStaticLimits.get(model);
        this._modelStaticLimits.set(model, newStaticLimit);
        const currentEffective = this._modelLimits.get(model);
        // Only raise effective if it was at or above old ceiling
        if (currentEffective === undefined || (oldStatic !== undefined && currentEffective >= oldStatic)) {
            this._modelLimits.set(model, newStaticLimit);
        }
        return { oldStatic, newStatic: newStaticLimit, effective: this._modelLimits.get(model) };
    }
     ...
     */
    acquireModelSlot(model, keyIndex) {
        if (!model) return true;
        const limit = this._modelLimits.get(model);
        // Unknown model → permissive (no blocking)
        if (limit === undefined) return true;

        // Per-key tracking when keyIndex provided (limit is per z.ai account/key).
        // Falls back to global tracking when called without keyIndex (tests, legacy).
        const trackingKey = keyIndex !== undefined ? `${keyIndex}:${model}` : model;
        const current = this._modelInFlight.get(trackingKey) || 0;
        if (current >= limit) return false;

        this._modelInFlight.set(trackingKey, current + 1);
        return true;
    }
     ...
     * @param {string} model - Target model name
     * @param {number} [keyIndex] - Key index for per-key tracking
     */
    releaseModelSlot(model, keyIndex) {
        if (!model) return;
        const trackingKey = keyIndex !== undefined ? `${keyIndex}:${model}` : model;
        const current = this._modelInFlight.get(trackingKey) || 0;
        if (current > 1) {
            ...
            this._modelInFlight.delete(trackingKey);
        }
    }

    /**
     * Get current in-flight count for a model.
     * If keyIndex provided: returns per-key count.
     * Otherwise: sums across all keys (for stats/logging).
     * @param {string} model - Target model name
     * @param {number} [keyIndex] - Key index for per-key lookup
     * @returns {number} Current in-flight count
     */
    getModelInFlight(model, keyIndex) {
        if (keyIndex !== undefined) {
            return this._modelInFlight.get(`${keyIndex}:${model}`) || 0;
        }
        // Sum across all per-key entries plus any legacy global entry
        let total = 0;
        const prefix = `:${model}`;
        for (const [key, count] of this._modelInFlight.entries()) {
            if (key === model || key.endsWith(prefix)) total += count;
        }
        return total;
    }
     ...
     * @param {string} model - Target model name
     * @param {number} [keyIndex] - Key index for per-key check
     * @returns {boolean} true if at capacity
     */
    isModelAtCapacity(model, keyIndex) {
        if (!model) return false;
        const limit = this._modelLimits.get(model);
        if (limit === undefined) return false;
        const trackingKey = keyIndex !== undefined ? `${keyIndex}:${model}` : model;
        return (this._modelInFlight.get(trackingKey) || 0) >= limit;
    }

    /**
     * Get per-model concurrency stats (aggregated across all keys)
     * @returns {Object} Map of model -> { inFlight, maxConcurrency }
     */
    getModelConcurrencyStats() {
        const stats = {};
        // Include all models that have limits
        for (const [model, limit] of this._modelLimits.entries()) {
            stats[model] = {
                inFlight: this.getModelInFlight(model),
                maxConcurrency: limit
            };
        }
        // Include models with in-flight but no configured limit
        for (const [key] of this._modelInFlight.entries()) {
            // key is either 'model' or 'keyIndex:model'
            const model = key.includes(':') ? key.slice(key.indexOf(':') + 1) : key;
            if (!stats[model]) {
                stats[model] = { inFlight: this.getModelInFlight(model), maxConcurrency: null };
            }
        }
        return stats;
    }
    ...
    /**
     * Get per-provider key health summary for dashboard observability.
     * @returns {Object} Map of provider -> { total, available, openCircuits, inFlight, errorRate }
     */
    getProviderHealthStats() {
        ...
        for (const key of this.keys) {
            const prov = key.provider || '__untagged__';
            if (!providers.has(prov)) {
                providers.set(prov, { total: 0, available: 0, openCircuits: 0, inFlight: 0, totalRequests: 0, failures: 0 });
            }
            const stats = providers.get(prov);
            stats.total++;
            stats.inFlight += key.inFlight;
            stats.totalRequests += key.totalRequests;
            stats.failures += (key.totalRequests - key.successCount - key.inFlight);
            if (this.isKeyAvailable(key)) {
                stats.available++;
            }
            if (key.circuitBreaker.state === STATES.OPEN) {
                stats.openCircuits++;
            }
        }

        // Convert to plain object with error rates
        const result = {};
        for (const [prov, stats] of providers) {
            const completed = stats.totalRequests - stats.inFlight;
            result[prov] = {
                total: stats.total,
                configured: prov !== '__untagged__' ? stats.total : 0,
                available: stats.available,
                openCircuits: stats.openCircuits,
                inFlight: stats.inFlight,
                errorRate: completed > 0
                    ? Math.round((stats.failures / completed) * 100 * 10) / 10
                    : 0
            };
        }
        // Meta: surface default provider name for dashboard UX
        result._meta = { defaultProvider: this.defaultProviderName || null };
        return result;
    }

...
// Maximum capacity for the LRU deduplication cache
const DEDUP_LRU_CAPACITY = 1000;
...
class WebhookManager {
    ...
    constructor(options = {}) {
        ...
        this._recentEvents = new LRUMap(DEDUP_LRU_CAPACITY);
        ...
        // SSRF validation: allow private/localhost URLs in dev/test
        this._allowPrivateUrls = options.allowPrivateUrls || false;
    ...
    loadWebhooks(endpoints) {
        this.endpoints = endpoints.filter(ep => {
            ...
            // SSRF protection: warn about private/reserved ranges (config-loaded endpoints
            // are admin-controlled; hard rejection is enforced in testWebhook/API paths)
            if (!this._allowPrivateUrls) {
                const validation = this._validateWebhookUrl(ep.url);
                if (!validation.valid) {
                    this._log('warn', `Webhook URL targets private/reserved address: ${ep.url} — ${validation.error}`);
                }
            }
    ...
    recordError(errorType) {
        ...
        // Hard cap: prevent unbounded growth before time-window filter
        if (this._errorTimestamps.length > 10000) {
            this._errorTimestamps = this._errorTimestamps.slice(-5000);
        }
     ...
     * Rejects private/reserved IPs and non-HTTP(S) schemes.
    ...
    _validateWebhookUrl(url) {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
            return { valid: false, error: `Scheme not allowed: ${parsed.protocol} — only http: and https: are permitted` };
        }

        // Check hostname against private/reserved IP ranges
        const hostname = parsed.hostname;
        if (this._isPrivateHost(hostname)) {
            return { valid: false, error: `Private/reserved IP not allowed: ${hostname}` };
        }
    ...
    _isPrivateHost(hostname) {
        // Strip IPv6 brackets — URL parser returns '[::1]' for http://[::1]/path
        let h = hostname;
        ...
        const privatePatterns = [
            ...
            /^fc/i,                             // IPv6 fc00::/7 ULA
            /^fe80/i,                           // IPv6 link-local
            /^::1$/,                            // IPv6 loopback
            /^::$/,                             // IPv6 unspecified
        ];

        // Also check common loopback hostnames
        const loopbackNames = ['localhost', 'ip6-localhost', 'ip6-loopback'];
        if (loopbackNames.includes(h.toLowerCase())) {
            return true;
        }

        return privatePatterns.some(pattern => pattern.test(h));
    }
    ...
    async testWebhook(url) {
        // Validate URL against SSRF before attempting delivery

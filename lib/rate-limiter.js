...
class RateLimiter {
    ...
    updateSettings(settings) {
        if (settings.requestsPerMinute !== undefined) {
            const val = Number(settings.requestsPerMinute);
            if (!Number.isFinite(val) || val < 0) {
                return { error: 'requestsPerMinute must be a non-negative finite number' };
            }
            this.requestsPerMinute = val;
        }
        if (settings.burst !== undefined) {
            const val = Number(settings.burst);
            if (!Number.isFinite(val) || val < 0) {
                return { error: 'burst must be a non-negative finite number' };
            }
            this.burst = val;
        }

...
class RequestStore {
    ...
    constructor(options = {}) {
        ...
        // Derive a proper 32-byte key via SHA-256 instead of weak space-padding.
        // The old code used: encryptionKey.padEnd(32).slice(0, 32)
        // which is cryptographically weak for short/long keys.
        if (this.encryptionKey) {
            this._derivedKey = crypto.createHash('sha256').update(this.encryptionKey).digest();
            // Keep the legacy padded key for migration of old encrypted data
            this._legacyKey = this.encryptionKey.padEnd(32).slice(0, 32);
        }
    ...
    _encrypt(data) {
        ...
        const cipher = crypto.createCipheriv('aes-256-cbc', this._derivedKey, iv);
    ...
    _decrypt(data) {
        ...
        // Try new (SHA-256 derived) key first
        try {
            const decipher = crypto.createDecipheriv('aes-256-cbc', this._derivedKey, iv);
        ...
        // Migration path: try legacy (padEnd) key for data encrypted before this fix
        try {
            const decipher = crypto.createDecipheriv('aes-256-cbc', this._legacyKey, iv);
            let decrypted = decipher.update(encrypted, 'hex', 'utf8');
            decrypted += decipher.final('utf8');
            return decrypted;
        } catch (_legacyErr) {
            return data; // Return as-is if both keys fail
        }
    }

...
class StatsAggregator {
    constructor(options = {}) {
        ...
        this.rateLimitTracking = {
            ...
            contextOverflowTransient: 0 // Transient overflow retries (all models at capacity)
        };
    ...
    /**
     * Load stats from persistent storage file
     * @returns {boolean} True if stats were loaded successfully, false otherwise
     */
    load() {
     ...
     * @param {Object} data - Usage data { requests, successes, failures, lastUsed }
     */
    ...
    recordModelUsage(model, data = {}) {
        ...
        if (!this.modelStats.has(model)) {
            ...
            if (this.modelStats.size >= MAX_TRACKED_MODELS) {
                if (!this._modelLimitWarned) {
                    this._modelLimitWarned = true;
                    if (this.logger) {
                        this.logger.warn(`Model tracking limit reached (${MAX_TRACKED_MODELS}), ignoring new model: ${model}`);
                    }
                }
                return;
            }
    ...
    /**
     * Record a transient context overflow event
     * Called when all models with sufficient context are temporarily at capacity
     */
    recordContextOverflowTransient() {
        this.rateLimitTracking.contextOverflowTransient++;
    }
    ...
    /**
     * Get full aggregated statistics for the /stats endpoint
     * @param {Object} keyManager - Key manager instance for key-specific stats
     * @param {number} uptime - Server uptime in seconds
     * @param {Object|null} queueStats - Optional queue statistics
     * @returns {Object} Complete stats object including client requests, latency, errors, etc.
     */

...
class ModelWindow {
    constructor(model, staticMax, floor) {
        ...
        this.adjustTimestamps = [];  // Ring buffer of last 10 adjustment timestamps
        this.history = [];  // Last 100 adjustments: { ts, from, to, reason, classification }
    }
}
...
class AdaptiveConcurrencyController {
    ...
    /**
     * Atomically switch mode, performing the appropriate transitions.
     * @param {string} newMode - 'observe_only' or 'enforce'
     * @returns {{ previousMode: string, currentMode: string }} transition result
     * @throws {Error} if newMode is invalid
     */
    setMode(newMode) {
        if (newMode !== 'observe_only' && newMode !== 'enforce') {
            throw new Error(`Invalid mode: ${newMode}. Must be "observe_only" or "enforce".`);
        }
        const oldMode = this.config.mode;
        this.config.mode = newMode;

        if (newMode === 'enforce' && oldMode !== 'enforce') {
            // Push current computed windows to keyManager
            for (const [model, w] of this._windows) {
                this._keyManager?.setEffectiveModelLimit?.(model, w.effectiveMax);
            }
        }

        if (newMode === 'observe_only' && oldMode === 'enforce') {
            this._keyManager?.restoreStaticLimits?.();
        }

        this._logger.info('Adaptive concurrency mode changed', { from: oldMode, to: newMode });
        return { previousMode: oldMode, currentMode: newMode };
    }

    /**
     * Update the static baseline for a model's AIMD window.
     * Called by RateLimitSync when upstream rate limits change.
     * Only raises effectiveMax if it was at or above the old ceiling.
     * @param {string} model - Model name
     * @param {number} newStaticMax - New static maximum (>= 1)
     * @returns {boolean} true if window existed and was updated
     */
    updateStaticBaseline(model, newStaticMax) {
        const w = this._windows.get(model);
        if (!w) return false;
        const oldStatic = w.staticMax;
        w.staticMax = newStaticMax;
        // Only raise effective if it was at or above old ceiling
        if (w.effectiveMax >= oldStatic) {
            w.effectiveMax = newStaticMax;
        }
        if (this.config.mode === 'enforce') {
            this._keyManager?.setEffectiveModelLimit?.(model, w.effectiveMax);
        }
        this._logger.info('AIMD static baseline updated', {
            model, oldStatic, newStatic: newStaticMax, effectiveMax: w.effectiveMax
        });
        return true;
    }
        ...
        if (w.congestionCount > 0) {
            ...
            if (classification === 'quota') {
                ...
                if (w.effectiveMax !== prev) {
                    ...
                    w.adjustTimestamps.push(now);
                    if (w.adjustTimestamps.length > 10) w.adjustTimestamps.shift();
                    if (this._isOscillating(w, now)) {
                        this._logger.warn('AIMD oscillation detected', {
                            model, effectiveMax: w.effectiveMax,
                            adjustments: w.adjustTimestamps.length,
                            reason: w.lastAdjustReason
                        });
                    }

                    this._recordHistory(w, now, prev, w.effectiveMax, w.lastAdjustReason, classification);
        ...
        if (w.successCount > 0 && (now - w.lastCongestionAt > this.config.recoveryDelayMs)) {
            ...
            if (growthAllowed && w.effectiveMax < w.staticMax) {
                ...
                if (w.effectiveMax !== prev) {
                    ...
                    this._recordHistory(w, now, prev, w.effectiveMax, 'additive_increase', null);
            ...
            if ((now - w.lastTrafficAt > this.config.idleTimeoutMs) && w.effectiveMax < w.staticMax) {
                ...
                if (w.effectiveMax !== prev) {
                    ...
                    this._recordHistory(w, now, prev, w.effectiveMax, 'idle_decay', null);
    ...
    /**
     * Check if a model window is oscillating (>3 adjustments in 30 seconds).
     * @param {ModelWindow} w
     * @param {number} now
     * @returns {boolean}
     */
    _isOscillating(w, now) {
        const windowMs = 30000;
        const threshold = 3;
        const recent = w.adjustTimestamps.filter(t => now - t < windowMs);
    ...
    /**
     * Record an adjustment event in the model window's history ring buffer.
     * @param {ModelWindow} w
     * @param {number} now - Timestamp of the adjustment
     * @param {number} oldMax - Previous effectiveMax
     * @param {number} newMax - New effectiveMax
     * @param {string} reason - Adjustment reason (e.g. 'decrease_congestion', 'additive_increase', 'idle_decay')
     * @param {string|null} classification - 429 classification if applicable
     */
    _recordHistory(w, now, oldMax, newMax, reason, classification) {
        w.history.push({ ts: now, from: oldMax, to: newMax, reason, classification: classification || null });

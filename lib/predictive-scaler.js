...
class PredictiveScaler {
  ...
  recordUsage(timestamp, metrics) {
    ...
    // Hard cap: prevent unbounded growth even with large historyWindow
  ...
  /**
   * Stop the scaler and release resources.
   * Idempotent - safe to call multiple times.
   */
  stop() {
    // Currently PredictiveScaler has no internal timers (the interval is
    // managed externally by ProxyServer._scalerInterval), but this method
    // exists so that any future internal timers are cleaned up here and so
    // that ProxyServer.shutdown() can call scaler.stop() uniformly.
    this.reset();
  }

  /**
   * Alias for stop() for API consistency with other components.
   */
  destroy() {
    this.stop();
  }
